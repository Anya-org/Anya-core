...
pub use bitcoin::taproot::{TapBranchHash, TapLeaf, TapTree, LeafVersion, TaprootBuilder, ControlBlock};
...
impl BIPRegistry {
    ...
    pub fn status(&self, bip: &str) -> BIPStatus {
        self.registry.get(bip).cloned().unwrap_or(BIPStatus::NotSupported)
    }
