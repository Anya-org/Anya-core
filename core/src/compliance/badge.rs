...
const BADGE_TEMPLATE: &str = "https://img.shields.io/badge/BDF-%.2f%%25-4BC51D?logo=bitcoin&style=for-the-badge";
...
pub fn generate_badge(format: &str) -> anyhow::Result<String> {
    ...
    Ok(match format.to_lowercase().as_str() {
        "svg" => badge_url,
        "md" => format!("[![BDF Compliance]({})](https://compliance.anya.btc)", badge_url),
...
fn calculate_compliance_score(report: &BipComplianceReport) -> f32 {
    ...
    weights.iter()
        ...
        .sum::<f32>() / weights.iter().map(|(_, w)| w).sum::<f32>()
} 