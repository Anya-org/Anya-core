...
impl Bip353 {
    ...
    pub fn parse_address(&self, address: &str) -> Result<PaymentRecipient, Bip353Error> {
        ...
        if self.config.status == Bip353Status::Disabled {
            return Err(Bip353Error::FeatureNotEnabled("BIP353 is disabled".to_string()));
        }
        
        // Check for bitcoin prefix (₿)
        let has_prefix = address.starts_with("₿");
        let address_without_prefix = if has_prefix {
            &address[1..]
        } else {
            address
        };
        ...
        if parts.len() != 2 {
            return Err(Bip353Error::ParseError("Invalid address format".to_string()));
        }
        ...
        let has_non_ascii = !user.chars().all(|c| c.is_ascii()) || !domain.chars().all(|c| c.is_ascii());
        if has_non_ascii && !self.is_feature_enabled("non_ascii_identifiers") {
            return Err(Bip353Error::FeatureNotEnabled("Non-ASCII identifiers not enabled".to_string()));
        }
    ...
    pub async fn resolve(&self, recipient: &mut PaymentRecipient) -> Result<String, Bip353Error> {
            ...
            for txt in record.txt_data() {
                ...
                if data.starts_with("bitcoin:") {
                    if payment_instruction.is_some() {
                        return Err(Bip353Error::ParseError("Multiple bitcoin: records found".to_string()));
                    }
        ...
        let instruction = payment_instruction.ok_or_else(|| {
            Bip353Error::ResolutionError("No bitcoin: record found".to_string())
        })?;
    ...
    fn is_feature_enabled(&self, feature: &str) -> bool {
        ...
        match feature {
            "non_ascii_identifiers" => {
                self.config.status == Bip353Status::Beta && self.config.beta_features.non_ascii_identifiers
            },
            "wildcard_records" => {
                self.config.status == Bip353Status::Beta && self.config.beta_features.wildcard_records
            },
            "oob_notifications" => {
                self.config.status == Bip353Status::Beta && self.config.beta_features.oob_notifications
            },
            "enhanced_privacy" => {
                self.config.status == Bip353Status::Beta && self.config.beta_features.enhanced_privacy
            },
            // Basic features available in both stable and beta
            "basic_resolution" => self.config.status != Bip353Status::Disabled,
            "dnssec_validation" => self.config.status != Bip353Status::Disabled && self.config.validate_dnssec,
            _ => false,
        }
    }
    ...
    fn cache_instruction(&self, key: &str, instruction: &str) {
        ...
        cache.retain(|_, entry| entry.timestamp.elapsed() < Duration::from_secs(self.config.cache_duration));
    }
