...
impl DnsResolver {
    ...
    pub async fn resolve_payment_username(&self, user: &str, domain: &str) -> DnsResult<Vec<String>> {
        // Check cache first
        ...
        let lookup_result = match timeout(Duration::from_secs(DNS_TIMEOUT_SECS), lookup_future).await {
            Ok(result) => result.map_err(|e| DnsResolverError::Resolution(e.to_string())),
            Err(_) => Err(DnsResolverError::Timeout(format!("DNS lookup for {} timed out", name))),
        }?;
    ...
    fn format_dns_name(&self, user: &str, domain: &str) -> DnsResult<Name> {
        ...
        Name::from_ascii(&dns_name)
            .map_err(|e| DnsResolverError::RecordFormat(e.to_string()))
    }
