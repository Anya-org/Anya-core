...
impl PerformanceTestRunner {
        ...
        for config in &self.configs {
            for component in &self.components {
                if component.name() == config.name || config.name == "all" {
                    println!("Running test: {} on component: {}", config.name, component.name());
    ...
    pub fn run_test(&mut self, test_name: &str) -> Result<()> {
        let config = self.configs.iter()
            ...
            .ok_or_else(|| PerfTestError::ConfigurationError(
                format!("Test configuration not found: {}", test_name)
            ))?;
...
impl Timer {
    ...
    pub fn elapsed_ms(&self) -> Result<u64> {
        match (self.start, self.end) {
            (Some(start), Some(end)) => {
                Ok(end.duration_since(start).as_millis() as u64)
            }
            (Some(start), None) => {
                Ok(Instant::now().duration_since(start).as_millis() as u64)
            }
