...
use crate::testing::performance::{
    MetricType, PerformanceTestable, Result, TestConfig, TestResult, Timer,
};
...
impl PerformanceTestable for DatabaseAccessTest {
    fn run_test(&self, config: &TestConfig) -> Result<TestResult> {
        ...
        parameters.insert(
            ...
            test.db.config.batch_size.to_string(),
        );
        parameters.insert(
            "use_prepared_statements".to_string(),
            test.db.config.use_prepared_statements.to_string(),
        );
        parameters.insert(
            "cache_size_mb".to_string(),
            test.db.config.cache_size_mb.to_string(),
        );
        parameters.insert(
            "key_space_size".to_string(),
            test.key_space_size.to_string(),
        );
        ...
        if stats.reads > 0 {
            metrics.insert(
                "avg_read_time_ms".to_string(),
                (stats.read_time_ms as f64) / (stats.reads as f64),
            );
        ...
        if stats.writes > 0 {
            metrics.insert(
                "avg_write_time_ms".to_string(),
                (stats.write_time_ms as f64) / (stats.writes as f64),
            );
        ...
        if stats.updates > 0 {
            metrics.insert(
                "avg_update_time_ms".to_string(),
                (stats.update_time_ms as f64) / (stats.updates as f64),
            );
        ...
        if stats.deletes > 0 {
            metrics.insert(
                "avg_delete_time_ms".to_string(),
                (stats.delete_time_ms as f64) / (stats.deletes as f64),
            );
