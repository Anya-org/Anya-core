...
use anya_core::bip::{
    BipHealthChecker, BitcoinConfig, Bip353, Bip353Config, Bip353Status
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Process command
    match cli.command.unwrap_or(Commands::Check { format: "text".to_string() }) {
        Commands::Check { format } => {
            ...
            match format.to_lowercase().as_str() {
                ...
                _ => {
                    ...
                    println!("");
                    println!("Overall Health: {}", if report.healthy { "HEALTHY" } else { "NEEDS ATTENTION" });
        ...
        Commands::Monitor { interval, output_dir } => {
            ...
            info!("Starting BIP health monitoring (interval: {} seconds)", interval);
