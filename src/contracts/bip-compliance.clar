...
;(define-read-only (verify-taproot-signature (msg (buff 1024;);); (sig (buff 64;);) (pubkey (buff 32;);););  (match (secp256k1-verify msg sig pubkey;)
    true (ok true;)
    false (err u1001;)
;)
;)
...
(define-public (process-psbt (psbt (buff 1024;);););  (let (
    (parsed (try! (parse-psbt psbt;);););    (finalized (try! (finalize-psbt parsed;););)
;);  (ok finalized;)
;)
;) 
