//! Legal wrapper integration for DAO-4 [AIS-3][BPC-3][DAO-4]
...
impl LegalWrapper {
    ...
    pub fn register_entity(&mut self, entity: LegalEntity) -> Result<(), LegalError> {
        ...
        if !self.jurisdictions.contains(&entity.jurisdiction) {
            return Err(LegalError::UnsupportedJurisdiction(
                format!("Jurisdiction not supported: {}", entity.jurisdiction)
            ));
        }
    ...
    pub fn verify_signature(&self, signature: &LegalSignature) -> Result<bool, LegalError> {
        ...
        if !self.jurisdictions.contains(&signature.jurisdiction) {
            return Err(LegalError::UnsupportedJurisdiction(
                format!("Jurisdiction not supported: {}", signature.jurisdiction)
            ));
        }
        ...
        if !entity.signatories.contains(&signature.signer) {
            return Err(LegalError::SignatureError(
                format!("Signer {} not authorized for entity {}", signature.signer, entity.name)
            ));
        }
        
        // Check Bitcoin anchoring for BPC-3 compliance
        let requirements = self.signature_requirements.get(&signature.jurisdiction)
            .ok_or_else(|| {
                LegalError::UnsupportedJurisdiction(
                    format!("No requirements for jurisdiction: {}", signature.jurisdiction)
                )
            })?;
