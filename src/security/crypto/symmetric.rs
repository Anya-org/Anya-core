...
impl SymmetricCrypto {
    ...
    pub fn encrypt(&self, key: &[u8], nonce: &[u8], plaintext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>, SymmetricError> {
        match self.algorithm {
            SymmetricAlgorithm::Aes256Gcm => {
                self.encrypt_aes_gcm(key, nonce, plaintext, aad)
            },
            SymmetricAlgorithm::ChaCha20Poly1305 => {
                self.encrypt_chacha20_poly1305(key, nonce, plaintext, aad)
            },
            _ => {
                Err(SymmetricError::EncryptionError(format!(
                    "Algorithm {:?} not yet implemented", self.algorithm
                )))
            }
        }
    }
    ...
    pub fn decrypt(&self, key: &[u8], nonce: &[u8], ciphertext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>, SymmetricError> {
        match self.algorithm {
            SymmetricAlgorithm::Aes256Gcm => {
            ...
            _ => {
                Err(SymmetricError::DecryptionError(format!(
                    "Algorithm {:?} not yet implemented", self.algorithm
                )))
            }
        }
    ...
    fn encrypt_aes_gcm(&self, key: &[u8], nonce: &[u8], plaintext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>, SymmetricError> {
        // Validate key and nonce
        if key.len() != 32 {
            return Err(SymmetricError::InvalidKeyError(
                format!("AES-256-GCM requires a 32-byte key, got {}", key.len())
            ));
        }
        
        if nonce.len() != 12 {
            return Err(SymmetricError::InvalidNonceError(
    ...
    fn decrypt_aes_gcm(&self, key: &[u8], nonce: &[u8], ciphertext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>, SymmetricError> {
        // Validate key and nonce
        if key.len() != 32 {
            return Err(SymmetricError::InvalidKeyError(
                format!("AES-256-GCM requires a 32-byte key, got {}", key.len())
    ...
    fn encrypt_chacha20_poly1305(&self, key: &[u8], nonce: &[u8], plaintext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>, SymmetricError> {
        // Validate key and nonce
        if key.len() != 32 {
            return Err(SymmetricError::InvalidKeyError(
                format!("ChaCha20-Poly1305 requires a 32-byte key, got {}", key.len())
            ));
        }
        
        if nonce.len() != 12 {
            return Err(SymmetricError::InvalidNonceError(
                format!("ChaCha20-Poly1305 requires a 12-byte nonce, got {}", nonce.len())
            ));
        }
    ...
    fn decrypt_chacha20_poly1305(&self, key: &[u8], nonce: &[u8], ciphertext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>, SymmetricError> {
        // Validate key and nonce
        if key.len() != 32 {
            return Err(SymmetricError::InvalidKeyError(
                format!("ChaCha20-Poly1305 requires a 32-byte key, got {}", key.len())
...
mod tests {
    ...
    fn test_helper_functions() {
        ...
        assert_eq!(chacha_crypto.algorithm, SymmetricAlgorithm::ChaCha20Poly1305);
    }
} 