...
impl From<&TypesHsmAuditEvent>
    for Result<ErrorHsmAuditEvent, crate::security::hsm::error::HsmError>
{
    fn from(event: &TypesHsmAuditEvent) -> Self {
        ...
        let mut audit_event = ErrorHsmAuditEvent::new(event_type, result, severity);

        // Add details if available
        if let Some(details) = &event.details {
            if let Ok(event_with_metadata) = audit_event
                .clone()
        ...
        if let Some(op_id) = &event.operation_id {
            if let Ok(event_with_params) = audit_event
                .clone()
                .with_parameters(&serde_json::json!({ "operation_id": op_id }))
            {
