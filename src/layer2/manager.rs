use crate::layer2::{
    Layer2Protocol, Layer2ProtocolTrait, Proof,
    BobClient, LiquidModule, RskClient, StacksClient, TaprootAssetsProtocol,
    LightningNetwork, StateChannel,
};
...
impl Layer2Manager {
    ...
    pub fn get_protocol(&self, protocol_type: Layer2Protocol) -> Option<&dyn Layer2ProtocolTrait> {
        match protocol_type {
            Layer2Protocol::BOB => self.bob_client.as_ref().map(|c| c as &dyn Layer2ProtocolTrait),
            Layer2Protocol::Liquid => self.liquid_module.as_ref().map(|c| c as &dyn Layer2ProtocolTrait),
            Layer2Protocol::RSK => self.rsk_client.as_ref().map(|c| c as &dyn Layer2ProtocolTrait),
            Layer2Protocol::Stacks => self.stacks_client.as_ref().map(|c| c as &dyn Layer2ProtocolTrait),
            Layer2Protocol::TaprootAssets => self.taproot_assets.as_ref().map(|c| c as &dyn Layer2ProtocolTrait),
    ...
    pub fn cross_layer_transfer(
        ...
        let transfer_id = format!("cross_{}_{}_{}_{}", 
    ...
    pub fn verify_cross_layer_proof(
        ...
        println!("Verifying cross-layer proof across {} protocols", protocols.len());
