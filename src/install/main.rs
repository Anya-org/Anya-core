...
fn install_core(config_path: &str, matches: clap::ArgMatches) -> Result<(), String> {
    ...
    if let Some(network) = &mut config.network {
        ...
        network.bitcoin_mainnet_rpc_url =
            crate::bitcoin::endpoints::DEFAULT_MAINNET_RPC.to_string();
        network.bitcoin_testnet_rpc_url =
        ...
        if let Some(custom_rpc) = matches.value_of("rpc-endpoint") {
            ...
            info!(
                "Using default Bitcoin {} RPC endpoint: {}",
...
fn generate_compliance_report() -> Result<(), String> {
    ...
    info!(
        "Installation compliance report generated in {}/compliance_report.md",
        report_dir
    );
    Ok(())
}
