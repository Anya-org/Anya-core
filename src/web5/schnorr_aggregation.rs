...
use bitcoin::secp256k1::schnorr::{KeyAggCoef, KeyAggContext, Signature};
...
impl SignatureAggregator {
    ...
    fn sign_inputs_individually(
        ...
        for input in inputs {
            ...
            // Create message
            let message =
                Message::from_slice(&sighash).map_err(|_| AggregationError::SigningFailed)?;
    ...
    fn cross_input_aggregation(
        ...
        let message =
            Message::from_slice(&combined_hash).map_err(|_| AggregationError::SigningFailed)?;
    ...
    fn musig_cross_input_aggregation(
        ...
        let agg_pubkey = self.approximate_musig_key_aggregation(&pubkeys)?;
    ...
    fn approximate_musig_key_aggregation(
        &self,
        pubkeys: &[PublicKey],
    ...
    pub fn verify_aggregated_signature(
        ...
        let pubkey = if self.mode == AggregationMode::CrossInputMuSig {
            ...
            self.approximate_musig_key_aggregation(&pubkeys)?
        } else {
        ...
        match self
            .secp
            .verify_schnorr(&aggregated.signature, &message, &pubkey)
        {
            Ok(_) => Ok(true),
            Err(_) => Ok(false),
        }
