        ...
        match output {
            Ok(output) => {
                if !output.status.success() {
                    ...
                    return Err(format!("Failed to load model {}: {}", model_path_str, error));
                }
                
                let validation_result = String::from_utf8_lossy(&output.stdout);
                info!("Model {} validated successfully: {}", model_path_str, validation_result);
            },
            Err(e) => return Err(format!("Failed to validate model {}: {}", model_path_str, e)),
        }
    }
