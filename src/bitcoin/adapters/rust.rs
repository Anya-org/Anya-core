...
use bdk::{
    ...
    wallet::{
        coin_selection::{CoinSelectionAlgorithm, DefaultCoinSelectionAlgorithm},
        AddressIndex,
    },
    FeeRate, SyncOptions, Wallet,
};
use bitcoin::{consensus, Address, Block, Network, Script, Transaction, Txid};
...
impl RustBitcoinImplementation {
    ...
    pub fn new(config: &crate::config::Config) -> Self {
        ...
        let network = match network_str.as_str() {
            ...
            _ => {
                println!(
                    "Warning: Unknown network '{}', defaulting to testnet",
                    network_str
                );
                Network::Testnet
            }
    ...
    fn initialize_wallet(&self) -> BitcoinResult<()> {
        ...
        let mnemonic = Mnemonic::generate(WordCount::Words12).map_err(|e| {
            BitcoinError::WalletError(format!("Failed to generate mnemonic: {}", e))
        })?;

        println!(
            "Generated new wallet with mnemonic: {}",
            mnemonic.to_string()
        );
        ...
        *self
            .mnemonic
            .lock()
            .map_err(|e| format!("Mutex lock error: {}", e))? = Some(mnemonic.clone());

        // Create extended key from mnemonic
        let xkey: ExtendedKey = mnemonic.into_extended_key().map_err(|e| {
            BitcoinError::WalletError(format!("Failed to create extended key: {}", e))
        })?;
        ...
        let receive_descriptor = Descriptor::new(receive_descriptor).map_err(|e| {
            BitcoinError::WalletError(format!("Failed to create receive descriptor: {}", e))
        })?;

        // Create a descriptor for change addresses
        let change_descriptor = format!("wpkh({}/1/*)", xprv);
        let change_descriptor = Descriptor::new(change_descriptor).map_err(|e| {
            BitcoinError::WalletError(format!("Failed to create change descriptor: {}", e))
        })?;
        ...
        *self
            .wallet
            .lock()
            .map_err(|e| format!("Mutex lock error: {}", e))? = Some(wallet);

        // Connect to Electrum server
        let external = crate::bitcoin::external_endpoints::ExternalBitcoinEndpoints::resolve();
        let electrum_url = match self.network {
            Network::Bitcoin => external.electrum_mainnet.as_str(),
            Network::Testnet => external.electrum_testnet.as_str(),
            _ => external.electrum_testnet.as_str(), // Default to testnet
        };
        ...
        let blockchain = ElectrumBlockchain::from_config(&config).map_err(|e| {
            BitcoinError::NetworkError(format!("Failed to connect to Electrum server: {}", e))
        })?;
        ...
        *self
            .blockchain
            .lock()
            .map_err(|e| format!("Mutex lock error: {}", e))? = Some(blockchain);

        // Sync the wallet if blockchain is available
        if let Some(blockchain) = &*self
            .blockchain
            .lock()
            .map_err(|e| format!("Mutex lock error: {}", e))?
        {
            if let Some(wallet) = &mut *self
                .wallet
                .lock()
                .map_err(|e| format!("Mutex lock error: {}", e))?
            {
                wallet
                    ...
                    .map_err(|e| {
                        BitcoinError::NetworkError(format!("Failed to sync wallet: {}", e))
                    })?;
...
impl BitcoinInterface for RustBitcoinImplementation {
    fn get_transaction(&self, txid: &str) -> BitcoinResult<BitcoinTransaction> {
        ...
        let blockchain = blockchain_guard.as_ref().ok_or_else(|| {
            BitcoinError::ImplementationError("Blockchain not initialized".to_string())
        })?;

        // Parse the transaction ID
        let tx_hash = Txid::from_str(txid).map_err(|e| {
            BitcoinError::TransactionError(format!("Invalid transaction ID: {}", e))
        })?;
    ...
    fn get_block_height(&self) -> BitcoinResult<u32> {
        ...
        let wallet = wallet_guard.as_ref().ok_or_else(|| {
            BitcoinError::ImplementationError("Wallet not initialized".to_string())
        })?;
        ...
        match wallet.sync(blockchain, SyncOptions::default()) {
            Ok(()) => {
                ...
                match wallet.get_last_synced_height() {
                    Ok(height) => Ok(height),
                    Err(e) => Err(BitcoinError::BlockError(format!(
    ...
    fn generate_address(&self, address_type: AddressType) -> BitcoinResult<BitcoinAddress> {
        ...
        let wallet = wallet_guard.as_mut().ok_or_else(|| {
            BitcoinError::ImplementationError("Wallet not initialized".to_string())
        })?;
        ...
        let bdk_address = match address_type {
            ...
            AddressType::P2WPKH => {
                ...
                wallet
                    ...
                    .map_err(|e| {
                        BitcoinError::WalletError(format!("Failed to generate address: {}", e))
                    })?
                    .address
            }
            AddressType::P2WSH => {
    ...
    fn create_transaction(
        ...
        for (addr, amount) in outputs {
            // Parse the address
            let address = Address::from_str(&addr).map_err(|e| {
                BitcoinError::TransactionError(format!("Invalid address {}: {}", addr, e))
            })?;
