...
impl NetworkValidator {
    ...
    pub async fn validate_network(&self) -> NetworkValidationResult {
        ...
        let (dns, bandwidth, latency, ports) = tokio::join!(
            dns_future,
            bandwidth_future,
            latency_future,
            ports_future
        ...
        let recommendations = self.generate_recommendations(
            &connectivity, &dns, &bandwidth, &latency, &ports, &ssl, &firewall, &route
        );
    ...
    async fn validate_bandwidth(&self) -> BandwidthValidationResult {
        ...
        let download_result = reqwest::get("https://speed.cloudflare.com/__down?bytes=5000000").await;
        ...
        let upload_result = client.post("https://speed.cloudflare.com/__up")
            .body(upload_bytes)
            .send()
            .await;
    ...
    async fn validate_ports(&self) -> PortValidationResult {
        ...
        for &port in bip341_ports.iter().chain(&self.config.required_ports) {
            let is_open = if port == 8433 {  // Special handling for Taproot port
    ...
    async fn validate_routes(&self) -> RouteValidationResult {
        ...
        let average_hops = if route_count > 0 { total_hops / route_count } else { 0 };
    ...
    async fn detect_vpn(&self) -> bool {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            match output {
                Ok(output) => {
                    let stdout = String::from_utf8_lossy(&output.stdout);
                    stdout.contains("VPN") || stdout.contains("Cisco") || stdout.contains("Global Protect")
                },
                Err(_) => false,
            }
        }
    ...
    fn generate_recommendations(
        ...
        } else if !connectivity.endpoints_reachable.iter().all(|(_, reachable)| *reachable) {
    ...
    fn determine_overall_status(
        ...
        let optional_warnings = ssl.as_ref().map_or(false, |r| r.status == ValidationStatus::Warning) ||
                               firewall.as_ref().map_or(false, |r| r.status == ValidationStatus::Warning) ||
                               route.as_ref().map_or(false, |r| r.status == ValidationStatus::Warning);
