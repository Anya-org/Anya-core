...
async fn test_transaction_submission() {
    ...
    // Use the mock
    let tx_id = anya_core::layer2::Layer2Protocol::submit_transaction(&mock_protocol, tx_data)
        .await
        .unwrap();
    assert!(!tx_id.is_empty(), "Transaction ID should not be empty");
    ...
    let status =
        anya_core::layer2::Layer2Protocol::check_transaction_status(&mock_protocol, &tx_id)
            .await
            .unwrap();
...
async fn test_layer2_performance_benchmarks() {
    ...
    for i in 0..num_transactions {
        ...
        let result = anya_core::layer2::Layer2Protocol::submit_transaction(
            &mock_protocol,
            tx_data.as_bytes(),
        )
        .await;
...
async fn test_protocol_state_synchronization() {
    ...
    let state = anya_core::layer2::Layer2Protocol::get_state(&mock)
        .await
        .unwrap();
    assert!(
        !state.version.is_empty(),
    ...
    let protocols: Vec<Box<dyn Layer2ProtocolTrait + Send + Sync>> = vec![Box::new(mock)];
    ...
    for protocol in protocols.iter() {
        ...
        println!(
            "Protocol trait state: version {}, height {}",
            state.version, state.height
        );
    }
}
