...
fn test_minimum_hardware_support(hw_manager: &Arc<HardwareOptimizationManager>) -> bool {
    ...
    let min_specs_path = Path::new(
        "C:/Users/bmokoka/Projects/anya-core/core/src/hardware_optimization/MINIMUM_SPECS.md",
    );
...
fn test_minimum_hardware_performance(hw_manager: &Arc<HardwareOptimizationManager>) -> bool {
    ...
    for _ in 0..iterations {
        hw_manager
            .optimize_operation(OptimizableOperation::SchnorrVerification, &[0u8; 32])
            .unwrap();
    }
...
fn test_consistent_validation(hw_manager: &Arc<HardwareOptimizationManager>) -> bool {
    ...
    let results = (0..5)
        .map(|_| validator.validate(&tx).is_ok())
        .collect::<Vec<_>>();
