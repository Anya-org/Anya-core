...
async fn test_hardware_detection(hw_manager: &HardwareOptimizationManager) {
    ...
    assert!(hardware_type == HardwareType::CPU || hardware_type == HardwareType::GPU, 
    ...
    let avx2_support = capabilities.vector_extensions
        .iter()
        .any(|ext| ext == "AVX2");
    println!("  AVX2 Support: {}", if avx2_support { "Yes" } else { "No" });
...
async fn test_intel_optimizations(intel_opt: &Arc<IntelOptimizer>) {
    ...
    println!("  Kaby Lake optimized: {}", if is_kaby_lake { "Yes" } else { "No" });
    println!("  Meets minimum requirements: {}", if meets_min_req { "Yes" } else { "No" });
    
    // Test optimal batch size calculation
    let batch_size = intel_opt.calculate_optimal_batch_size();
    println!("  Calculated optimal batch size: {}", batch_size);
    if is_kaby_lake {
        assert_eq!(batch_size, 384, "Kaby Lake optimal batch size should be 384");
    }
    ...
    if is_kaby_lake {
        ...
        assert!(taproot_result.is_ok(), "Taproot verification should succeed");
    }
...
async fn test_batch_verification_optimizations(hw_manager: &HardwareOptimizationManager) {
    ...
    for batch_size in BATCH_SIZES.iter() {
        ...
        println!("  Batch size {}: Processed {} transactions in {:.2?} ({:.2} tx/sec)", 
...
async fn test_dlc_oracle_batch_verification(hw_manager: &HardwareOptimizationManager) {
    ...
    println!("  Processed {} DLC oracle verifications in {:.2?} ({:.2} verifications/sec)", 
...
async fn test_adaptive_work_scheduling(hw_manager: &HardwareOptimizationManager) {
    ...
    println!("  Work items processed: {}/{}", metrics.items_processed, WORK_ITEMS);
    println!("  Processing throughput: {:.2} items/sec", 
    ...
    assert!(metrics.items_processed > 0, "Should process at least some work items");
    assert!(metrics.work_steals > 0, "Should have some work stealing events");
...
async fn test_bitcoin_consensus_compliance(hw_manager: &HardwareOptimizationManager) {
    ...
    let standard_validator = TransactionValidator::new()
        .with_optimization(false); // Disable optimizations
        
    let optimized_validator = TransactionValidator::new()
        .with_optimization(true);  // Enable optimizations
    ...
    for (i, tx) in transactions.iter().enumerate() {
        ...
        assert_eq!(standard_result.is_ok(), optimized_result.is_ok(),
                  "Transaction {} validation results don't match", i);
    }
    ...
    for (i, tx) in transactions.iter().enumerate() {
        ...
        // Results must match to maintain consensus
        assert_eq!(standard_result.is_ok(), optimized_result.is_ok(),
                  "Taproot transaction {} validation results don't match", i);
    }
