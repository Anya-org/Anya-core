...
pub fn run_all_tests() -> Result<HashMap<String, bool>> {
    ...
    // Run BIP-341 tests
    suite.run_test("BIP-341: Key Path Spending", || test_taproot_key_path_spending());
    suite.run_test("BIP-341: Script Path Spending", || test_taproot_script_path_spending());
    suite.run_test("BIP-341: Multisig with Schnorr", || test_taproot_multisig_schnorr());
    suite.run_test("BIP-341: Edge Cases", || test_taproot_edge_cases());
    suite.run_test("BIP-341: Compliance Vectors", || test_taproot_compliance_vectors());
