...
use bitcoin::taproot::{TapBranchHash, TapLeaf, TapTree, LeafVersion, TaprootBuilder};
...
fn test_taproot_key_path_spending() -> Result<()> {
    ...
    let internal_key = XOnlyPublicKey::from_str("93c7378d96518a75448821c4f7c8f4bae7ce60f804d03d1f0628dd5dd0f5de51")?;
    
    // Create spending conditions
    let script = ScriptBuf::from_hex("5121030681b3e0d62e8455f48c657bf8b2556e1c6c89be232f57f1f53a88b0a9986cc751ae")?;
    let leaf = TapLeaf::new(LeafVersion::TapScript, script);
    
    // Create Taproot tree
    let tap_tree = TapTree::builder().add_leaf(0, leaf.clone()).finalize(&secp, internal_key)?;
    
    // Get Taproot output key
    let tap_output_key = tap_tree.output_key();
    
    // Verify tap output key matches expected value
    let expected_key = XOnlyPublicKey::from_str("ee4fe085983462a184015d1f782d6a5f8b9c2b60130aff050ce221aff7cc6b47")?;
...
fn test_taproot_script_path_spending() -> Result<()> {
    ...
    let script1 = ScriptBuf::from_hex("5121030681b3e0d62e8455f48c657bf8b2556e1c6c89be232f57f1f53a88b0a9986cc751ae")?;
    let script2 = ScriptBuf::from_hex("5121036e34cc5ee5558b925045f968e834316d8c90c8d0dd850cc3f990d56755abfa0751ae")?;
...
fn test_taproot_multisig_schnorr() -> Result<()> {
    ...
    let key1 = XOnlyPublicKey::from_str("79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798")?;
    let key2 = XOnlyPublicKey::from_str("c6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee5")?;
    let key3 = XOnlyPublicKey::from_str("e493dbf1c10d80f3581e4904930b1404cc6c13900ee0758474fa94abe8c4cd13")?;
...
fn test_taproot_edge_cases() -> Result<()> {
    ...
    let tap_tree_max = TapTree::builder().add_leaf(0, max_leaf.clone()).finalize(&secp, internal_key)?;
...
fn test_taproot_compliance_vectors() -> Result<()> {
    ...
    let internal_pubkey = XOnlyPublicKey::from_str("d6889cb081036e0faefa3a35157ad71086b123b2b144b649798b494c300a961d")?;
    let merkle_root = TapBranchHash::from_str("53a1f6e454df1aa2776a2814a721372d6258050de330b3c6d10ee8f4e0dda343")?;
    ...
    let expected_output_key = XOnlyPublicKey::from_str("53a1f6e454df1aa2776a2814a721372d6258050de330b3c6d10ee8f4e0dda343")?;
    
    // Build verifier and check
    let verifier = TaprootVerifier::new();
    let (output_key, parity) = verifier.compute_taproot_output_key(&internal_pubkey, tweak.as_ref())?;
    
    assert_eq!(output_key, expected_output_key, "Taproot output key doesn't match expected BIP-341 test vector");
