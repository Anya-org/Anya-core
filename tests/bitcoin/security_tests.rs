use anya_core::{
    bitcoin::{
        ...
        consensus::{
            ConsensusError, 
            differential_fuzzer::{DifferentialFuzzer, DifferentialFuzzerConfig, BitcoinReferenceClient},
            invariant_checker::{BitcoinCoreInvariantChecker, verify_transaction_consensus_invariants}
        }
    },
    hardware_optimization::HardwareOptimizationManager
};

use bitcoin::{Transaction, Block, BlockHeader, Script, OutPoint, TxIn, TxOut};
use std::sync::Arc;
...
fn test_cve_2010_5139_value_overflow() {
    ...
    assert!(result.is_err(), "Value overflow transaction should be rejected");
...
fn test_cve_2018_17144_duplicate_inputs() {
    ...
    let result = validator.validate(&exploit_tx);
    assert!(result.is_err(), "Transaction with duplicate inputs should be rejected");
    
    // Check invariant validation also catches this
    let invariant_result = verify_transaction_consensus_invariants(&exploit_tx);
    assert!(invariant_result.is_err(), "Invariant checker should reject duplicate inputs");
...
fn test_consensus_invariant_checker() {
    ...
    assert!(!invariants.is_empty(), "Should have at least one invariant defined");
    
    // Test with valid transaction
    let valid_tx = create_valid_transaction();
    let result = checker.check_transaction(&valid_tx);
    assert!(result.is_ok(), "Valid transaction should pass invariant checks");
...
fn test_hardware_optimizations_consensus() {
    ...
    for (i, tx) in transactions.iter().enumerate() {
        ...
        assert_eq!(standard_result.is_ok(), optimized_result.is_ok(),
                  "Transaction {} validation results differ: standard={:?}, optimized={:?}",
                  i, standard_result, optimized_result);
    }
...
fn test_timing_side_channels() {
    ...
    let avg_valid: f64 = valid_times.iter().map(|t| t.as_nanos() as f64).sum::<f64>() / iterations as f64;
    let avg_invalid: f64 = invalid_times.iter().map(|t| t.as_nanos() as f64).sum::<f64>() / iterations as f64;
    ...
    println!("Valid tx avg time: {:.2} ns, std dev: {:.2}", avg_valid, std_dev_valid);
    println!("Invalid tx avg time: {:.2} ns, std dev: {:.2}", avg_invalid, std_dev_invalid);
    ...
    assert!(diff_ratio < 0.5, "Timing difference may reveal too much information");
...
fn test_historical_consensus_bugs() {
    ...
    assert!(validator.validate(&tx).is_err(), "Should reject value overflow");
    
    // Test CVE-2018-17144 (inflation bug)
    println!("  Testing CVE-2018-17144...");
    let tx = create_duplicate_inputs_transaction();
    assert!(validator.validate(&tx).is_err(), "Should reject duplicate inputs");
    ...
    assert!(validator.validate(&tx).is_err(), "Should reject mutated transaction");
...
fn test_invariant_violations(checker: &BitcoinCoreInvariantChecker) {
    ...
    assert!(checker.check_transaction(&invalid_version_tx).is_err(), 
    ...
    assert!(checker.check_transaction(&empty_inputs_tx).is_err(), 
    ...
    assert!(checker.check_transaction(&empty_outputs_tx).is_err(), 
    ...
    assert!(checker.check_transaction(&duplicate_inputs_tx).is_err(), 
