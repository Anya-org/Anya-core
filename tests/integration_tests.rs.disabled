use anya_core::{
    blockchain::{
        bitcoin::BitcoinOperations, lightning::LightningOperations, stacks::StacksOperations,
    },
    config::Config,
    ml_logic::{
        blockchain_integration::BlockchainIntegration, dao_rules::DAORule, mlfee::MLFeeManager,
    },
    user_management::{User, UserManager, UserRole},
};
...
mod tests {
    ...
    async fn common_setup(
        ...
        let (bitcoin_ops, stacks_ops, lightning_ops) =
            initialize_blockchain_operations(config).await?;
    ...
    async fn initialize_blockchain_operations(
        config: &Config,
    ) -> Result<(BitcoinOperations, StacksOperations, LightningOperations)> {
    ...
    async fn create_test_user(user_manager: &UserManager) -> Result<User> {
        user_manager
            .create_user("test_user", "password123", UserRole::Standard)
            .await
    }
