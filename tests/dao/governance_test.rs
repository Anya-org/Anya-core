#![feature(edition2021)]
use anya_core::dao::{
    DaoGovernance, DaoLabel, ProposalStatus, VoteDirection, 
    CrossChainGovernance, VerificationMethod, ComplianceLevel
};
...
fn test_dao3_compliance() {
    ...
    assert!(dao.verify_dao3_compliance(), "DAO should be DAO-3 compliant by default");
    
    // Test all components individually
    assert!(dao.quadratic_voting_enabled, "Quadratic voting should be enabled");
    assert!(dao.delegated_authority.active, "Delegated authority should be active");
    assert!(dao.cross_chain_governance.is_some(), "Cross-chain governance should be configured");
    assert!(dao.legal_wrappers.is_implemented(), "Legal wrappers should be implemented");
}
...
fn test_dao4_compliance() {
    ...
    assert!(!dao.verify_dao4_compliance(), "New DAO should not be DAO-4 compliant");
    
    // Upgrade legal wrappers to advanced
    dao.legal_wrappers.set_compliance_level(ComplianceLevel::Advanced);
    ...
    assert!(dao.verify_dao4_compliance(), "DAO should now be DAO-4 compliant");
}
...
fn test_delegation() {
    ...
    let proposal = dao.get_proposal(proposal_id).expect("Failed to get proposal");
    assert!(proposal.votes.contains_key("0xdelegate"), "Vote should be recorded under delegate");
    
    let vote = proposal.votes.get("0xdelegate").expect("Vote not found");
    assert!(vote.is_delegated, "Vote should be marked as delegated");
    assert_eq!(vote.delegated_from.as_ref().expect("Missing delegation source"), "0xdelegator");
    
    // With quadratic voting and 0.8 power factor:
    // sqrt(100) * 0.8 ≈ 8.0
    assert!((vote.power - 8.0).abs() < 0.1, "Power calculation is incorrect");
}
...
fn test_cross_chain_proposal() {
    ...
    let proposal_id = dao.create_cross_chain_proposal(
        "RSK Integration",
...
fn test_legal_wrappers() {
    ...
    // Add legal document
    dao.legal_wrappers.add_document("terms", "https://example.com/terms");
    assert!(dao.legal_wrappers.legal_documents.contains_key("terms"));
    
    // Upgrade compliance level
    dao.legal_wrappers.set_compliance_level(ComplianceLevel::Full);
