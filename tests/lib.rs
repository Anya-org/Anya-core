...
mod tests {
    ...
    async fn test_mobile_network() {
        ...
        let recipient = "bc1q09vm5lfy0j5reeulh4x5752ryscq5tun4677xu";
        let amount = 100000; // 1 BTC
        let tx = sdk.network.create_transaction(sender, recipient, amount).await.unwrap();
        assert!(!tx.is_empty());
    ...
    async fn test_tenant_manager() {
        ...
        let ip = "192.168.1.1";
        let permission = "read";
        assert!(manager.check_access(&tenant.id, ip, permission).await.unwrap());
        
        // Test tenant limit
        for _ in 0..9 {
            manager.create_tenant(&format!("Test Tenant {}", _)).await.unwrap();
        }
