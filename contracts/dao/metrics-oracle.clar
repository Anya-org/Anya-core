...
(define-constant ERR_INVALID_PARAMETER u402;);(define-constant ERR_STALE_DATA u403;);
(define-constant ERR_RATE_LIMITED u404;);(define-constant ERR_ORACLE_OFFLINE u405;)
...
(map-set administrators tx-sender true;);(map-set oracle-providers tx-sender true;)

;; Metric Types
(define-constant METRIC_TYPE_TOKEN u1;);
(define-constant METRIC_TYPE_TREASURY u2;);(define-constant METRIC_TYPE_GOVERNANCE u3;);
...
(map-set valid-metric-names { metric-name: "circulating_supply" } { metric-type: METRIC_TYPE_TOKEN };);(map-set valid-metric-names { metric-name: "total_minted" } { metric-type: METRIC_TYPE_TOKEN };);(map-set valid-metric-names { metric-name: "total_burned" } { metric-type: METRIC_TYPE_TOKEN };);(map-set valid-metric-names { metric-name: "velocity" } { metric-type: METRIC_TYPE_TOKEN };);(map-set valid-metric-names { metric-name: "active_wallets" } { metric-type: METRIC_TYPE_TOKEN };);(map-set valid-metric-names { metric-name: "holder_distribution" } { metric-type: METRIC_TYPE_TOKEN };);(map-set valid-metric-names { metric-name: "median_balance" } { metric-type: METRIC_TYPE_TOKEN };)
;(map-set valid-metric-names { metric-name: "reserves_ratio" } { metric-type: METRIC_TYPE_TREASURY };);(map-set valid-metric-names { metric-name: "pol_percentage" } { metric-type: METRIC_TYPE_TREASURY };);(map-set valid-metric-names { metric-name: "protocol_revenue" } { metric-type: METRIC_TYPE_TREASURY };);(map-set valid-metric-names { metric-name: "buyback_volume" } { metric-type: METRIC_TYPE_TREASURY };);(map-set valid-metric-names { metric-name: "runway_months" } { metric-type: METRIC_TYPE_TREASURY };);(map-set valid-metric-names { metric-name: "investment_performance" } { metric-type: METRIC_TYPE_TREASURY };);(map-set valid-metric-names { metric-name: "collateralization_ratio" } { metric-type: METRIC_TYPE_TREASURY };)
...
;(map-set valid-metric-names { metric-name: "price_7d_change" } { metric-type: METRIC_TYPE_MARKET };);(map-set valid-metric-names { metric-name: "liquidity_depth" } { metric-type: METRIC_TYPE_MARKET };);(map-set valid-metric-names { metric-name: "volume_24h" } { metric-type: METRIC_TYPE_MARKET };);(map-set valid-metric-names { metric-name: "volatility_index" } { metric-type: METRIC_TYPE_MARKET };);(map-set valid-metric-names { metric-name: "correlation_btc" } { metric-type: METRIC_TYPE_MARKET };);(map-set valid-metric-names { metric-name: "sentiment_score" } { metric-type: METRIC_TYPE_MARKET };)

;; Public Functions

;; Submit a token metric
(define-public (submit-token-metric (metric-name (string-ascii 64;);); (value uint;) (confidence uint;) (source (string-ascii 64;);););  (begin
...
(define-public (submit-treasury-metric (metric-name (string-ascii 64;);); (value uint;) (confidence uint;) (source (string-ascii 64;);););  (begin
    ;; Check oracle provider authorization
    (asserts! (is-oracle-provider tx-sender;) (err ERR_UNAUTHORIZED;);)
...
(define-public (submit-governance-metric (metric-name (string-ascii 64;);); (value uint;) (confidence uint;) (source (string-ascii 64;);););  (begin
    ;; Check oracle provider authorization
    (asserts! (is-oracle-provider tx-sender;) (err ERR_UNAUTHORIZED;);)
...
(define-public (submit-market-metric (metric-name (string-ascii 64;);); (value uint;) (confidence uint;) (source (string-ascii 64;);););  (begin
    ;; Check oracle provider authorization
    (asserts! (is-oracle-provider tx-sender;) (err ERR_UNAUTHORIZED;);)
...
;; Batch submit metrics (for gas efficiency;);(define-public (batch-submit-metrics 
                (token-metrics-list (list 10 {name: (string-ascii 64;), value: uint, confidence: uint};););
                (treasury-metrics-list (list 10 {name: (string-ascii 64;), value: uint, confidence: uint};););                (governance-metrics-list (list 10 {name: (string-ascii 64;), value: uint, confidence: uint};););                (market-metrics-list (list 10 {name: (string-ascii 64;), value: uint, confidence: uint};););                (source (string-ascii 64;);););  (begin
    ;; Check oracle provider authorization
    (asserts! (is-oracle-provider tx-sender;) (err ERR_UNAUTHORIZED;);)
    ...
    (map process-token-metric token-metrics-list;);    (map process-treasury-metric treasury-metrics-list;);    (map process-governance-metric governance-metrics-list;);    (map process-market-metric market-metrics-list;)
...
(define-private (process-token-metric (metric {name: (string-ascii 64;), value: uint, confidence: uint};););
  (if (is-valid-metric (get name metric;) METRIC_TYPE_TOKEN;);      (map-set token-metrics
...
;(define-private (process-treasury-metric (metric {name: (string-ascii 64;), value: uint, confidence: uint};););
  (if (is-valid-metric (get name metric;) METRIC_TYPE_TREASURY;);      (map-set treasury-metrics
        { metric-name: (get name metric;) }
        {
          value: (get value metric;),
          last-updated: block-height,
...
;(define-private (process-governance-metric (metric {name: (string-ascii 64;), value: uint, confidence: uint};););
  (if (is-valid-metric (get name metric;) METRIC_TYPE_GOVERNANCE;);      (map-set governance-metrics
        { metric-name: (get name metric;) }
        {
          value: (get value metric;),
          last-updated: block-height,
...
;(define-private (process-market-metric (metric {name: (string-ascii 64;), value: uint, confidence: uint};););
  (if (is-valid-metric (get name metric;) METRIC_TYPE_MARKET;);      (map-set market-metrics
        { metric-name: (get name metric;) }
        {
          value: (get value metric;),
          last-updated: block-height,
...
(define-read-only (get-token-metric (metric-name (string-ascii 64;);););
  (let ((metric (map-get? token-metrics { metric-name: metric-name };);););    (if (and 
          (is-some metric;) ;          (< (- block-height (get last-updated (unwrap-panic metric;););) (var-get data-freshness-threshold;);););        (ok (unwrap-panic metric;););        (err ERR_STALE_DATA;)
;)
;);)

;; Get treasury metric
(define-read-only (get-treasury-metric (metric-name (string-ascii 64;);););
  (let ((metric (map-get? treasury-metrics { metric-name: metric-name };);););    (if (and 
          (is-some metric;) ;          (< (- block-height (get last-updated (unwrap-panic metric;););) (var-get data-freshness-threshold;);););        (ok (unwrap-panic metric;););        (err ERR_STALE_DATA;)
;)
;);)
...
(define-read-only (get-governance-metric (metric-name (string-ascii 64;);););
  (let ((metric (map-get? governance-metrics { metric-name: metric-name };);););    (if (and 
          (is-some metric;) ;          (< (- block-height (get last-updated (unwrap-panic metric;););) (var-get data-freshness-threshold;);););        (ok (unwrap-panic metric;););        (err ERR_STALE_DATA;)
;)
;);)
...
(define-read-only (get-market-metric (metric-name (string-ascii 64;);););
  (let ((metric (map-get? market-metrics { metric-name: metric-name };);););    (if (and 
          (is-some metric;) ;          (< (- block-height (get last-updated (unwrap-panic metric;););) (var-get data-freshness-threshold;);););        (ok (unwrap-panic metric;););        (err ERR_STALE_DATA;)
;)
;);)
...
(define-read-only (is-valid-metric (metric-name (string-ascii 64;);); (expected-type uint;););  (match (map-get? valid-metric-names { metric-name: metric-name };)
...
;; Add a metric
(define-public (add-metric (metric-name (string-ascii 64;);); (metric-type uint;););  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;););    (asserts! (and (>= metric-type METRIC_TYPE_TOKEN;) (<= metric-type METRIC_TYPE_MARKET;);) (err ERR_INVALID_PARAMETER;);)
...
(define-public (add-oracle-provider (provider principal;););
  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;););    (map-set oracle-providers provider true;);    (ok true;)
;);)

;; Remove an oracle provider
(define-public (remove-oracle-provider (provider principal;););
  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;););    (map-set oracle-providers provider false;);    (ok true;)
;);)
...
(define-public (update-oracle-settings 
               ...
               (new-update-interval (optional uint;););               (new-data-freshness-threshold (optional uint;););               (new-oracle-status (optional (string-ascii 20;););););  (begin
