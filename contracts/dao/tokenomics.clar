...
;; Tokenomics Constants (Updated;);(define-constant TOTAL_SUPPLY u21000000000000000;) ;; 21B tokens with 8 decimals
...
(define-data-var halving-interval uint DEFAULT_HALVING_INTERVAL;);(define-data-var last-halving-block uint u0;);
...
(define-public (initialize-distribution;);
  (begin
    ...
    (let (
      ...
      (team-initial u0;) ;; 0% initial (cliff;);      (community-initial (/ (* COMMUNITY_ALLOCATION u10;) u100;);) ;; 10% initial
      (partners-initial (/ (* PARTNERS_ALLOCATION u10;) u100;);) ;; 10% initial;)
      ;; Track released amounts
      (var-set treasury-released treasury-initial;);      (var-set liquidity-released liquidity-initial;);      (var-set team-released team-initial;);      (var-set community-released community-initial;);      (var-set partners-released partners-initial;)
...
(define-public (process-vesting-release;);
  (begin
    ...
    (let (
      (blocks-since-launch (- block-height (var-get launch-timestamp;);););      (months-since-launch (/ blocks-since-launch u4320;);) ;; ~30 days with 10-min blocks;)
...
(define-public (process-block-rewards;);
  (begin
    ...
    (if (should-halve;);        (perform-halving;)
        true;)
...
(define-public (update-treasury-parameters (new-reserve-ratio uint;); (new-pol-ratio uint;););  (begin
    ...
    ;; Validate parameters
    (asserts! (and (>= new-reserve-ratio u5;) (<= new-reserve-ratio u30;);) (err ERR_INVALID_PARAMETER;););    (asserts! (and (>= new-pol-ratio u5;) (<= new-pol-ratio u30;);) (err ERR_INVALID_PARAMETER;);)
...
(define-public (update-strategic-reserves (amount uint;); (operation uint;););  (begin
    ;; Only the DAO contract can update treasury reserves
    (asserts! (is-eq contract-caller DAO_CONTRACT;) (err ERR_UNAUTHORIZED;);)
    
    ;; 1 = add, 2 = subtract
    (if (is-eq operation u1;);        (var-set strategic-reserves (+ (var-get strategic-reserves;) amount;););        (var-set strategic-reserves (- (var-get strategic-reserves;) amount;););)
    ;    (ok true;)
;);)

;; Update protocol-owned liquidity amount
(define-public (update-protocol-liquidity (amount uint;); (operation uint;););  (begin
    ;; Only the DAO contract can update protocol liquidity
    (asserts! (is-eq contract-caller DAO_CONTRACT;) (err ERR_UNAUTHORIZED;);)
    
    ;; 1 = add, 2 = subtract
    (if (is-eq operation u1;);        (var-set protocol-owned-liquidity (+ (var-get protocol-owned-liquidity;) amount;););        (var-set protocol-owned-liquidity (- (var-get protocol-owned-liquidity;) amount;););)
    ;    (ok true;)
;);)
...
(define-read-only (get-months-since-launch;);
  (if (> (var-get launch-timestamp;) u0;);      (/ (- block-height (var-get launch-timestamp;);) u4320;)
      u0;);)
...
(define-private (process-treasury-vesting (months uint;););
  (let (
    ...
    (vested-amount (if (>= months vesting-months;)
                       vesting-amount
                       (/ (* vesting-amount months;) vesting-months;);););    (total-vested (+ initial-amount vested-amount;););    (to-release (- total-vested (var-get treasury-released;););)
;);    (if (> to-release u0;);        (begin
          (var-set treasury-released total-vested;);          (var-set total-minted (+ (var-get total-minted;) to-release;);)
          true;)
        true;););)
;(define-private (process-liquidity-vesting (months uint;););
  (let (
    ...
    (vested-amount (if (>= months vesting-months;)
                       vesting-amount
                       (/ (* vesting-amount months;) vesting-months;);););    (total-vested (+ initial-amount vested-amount;););    (to-release (- total-vested (var-get liquidity-released;););)
;);    (if (> to-release u0;);        (begin
          (var-set liquidity-released total-vested;);          (var-set total-minted (+ (var-get total-minted;) to-release;);)
          true;)
        true;););)
;(define-private (process-team-vesting (months uint;););
  (let (
    ...
    (effective-months (if (>= months cliff-months;);                          (- months cliff-months;)
                          u0;););    (vested-amount (if (>= effective-months vesting-months-after-cliff;)
                       vesting-amount
                       (/ (* vesting-amount effective-months;) vesting-months-after-cliff;);););    (total-vested (+ initial-amount vested-amount;););    (to-release (- total-vested (var-get team-released;););)
;);    (if (> to-release u0;);        (begin
          (var-set team-released total-vested;);          (var-set total-minted (+ (var-get total-minted;) to-release;);)
          true;)
        true;););)
;(define-private (process-community-vesting (months uint;););
  (let (
    ...
    (vested-amount (if (>= months vesting-months;)
                       vesting-amount
                       (/ (* vesting-amount months;) vesting-months;);););    (total-vested (+ initial-amount vested-amount;););    (to-release (- total-vested (var-get community-released;););)
;);    (if (> to-release u0;);        (begin
          (var-set community-released total-vested;);          (var-set total-minted (+ (var-get total-minted;) to-release;);)
          true;)
        true;););)
;(define-private (process-partners-vesting (months uint;););
  (let (
    ...
    (vested-amount (if (>= months vesting-months;)
                       vesting-amount
                       (/ (* vesting-amount months;) vesting-months;);););    (total-vested (+ initial-amount vested-amount;););    (to-release (- total-vested (var-get partners-released;););)
;);    (if (> to-release u0;);        (begin
          (var-set partners-released total-vested;);          (var-set total-minted (+ (var-get total-minted;) to-release;);)
          true;)
        true;););)
