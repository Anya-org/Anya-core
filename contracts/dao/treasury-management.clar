...
(define-constant ERR_BELOW_THRESHOLD u404)
(define-constant ERR_EMERGENCY_ACTIVE u405)
...
(define-constant LIQUIDITY_PRINCIPAL 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.liquidity)
(define-constant STRATEGIC_RESERVES_PRINCIPAL 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.reserves)
...
(define-data-var pol-ratio uint POL_RATIO_TARGET)
...
(define-data-var emergency-council-size uint u3) ;; Number of signatures needed
...
(define-public (execute-buyback (amount uint) (proposal-id uint))
  (begin
    ...
    (let (
      ...
      (try! (contract-call? TOKENOMICS_CONTRACT update-strategic-reserves amount u2))
...
(define-public (add-protocol-liquidity (amount uint) (proposal-id uint))
  (begin
    ;; Only the DAO contract can execute operations
    (asserts! (or (is-eq contract-caller DAO_CONTRACT) (is-administrator tx-sender)) (err ERR_UNAUTHORIZED))
    ...
    (let (
      ...
      ;; Update treasury tracking
      (try! (contract-call? TOKENOMICS_CONTRACT update-protocol-liquidity amount u1))
      (var-set strategic-reserves (- (var-get strategic-reserves) amount))
...
(define-public (remove-protocol-liquidity (amount uint) (proposal-id uint))
  (begin
    ;; Only the DAO contract can execute operations
    (asserts! (or (is-eq contract-caller DAO_CONTRACT) (is-administrator tx-sender)) (err ERR_UNAUTHORIZED))
    ...
    (let (
      ...
      ;; Update treasury tracking
      (try! (contract-call? TOKENOMICS_CONTRACT update-protocol-liquidity amount u2))
      (var-set strategic-reserves (+ (var-get strategic-reserves) amount))
      (var-set protocol-owned-liquidity (- (var-get protocol-owned-liquidity) amount))
...
(define-public (execute-grant (recipient principal) (amount uint) (proposal-id uint))
  (begin
    ;; Only the DAO contract can execute operations
    (asserts! (or (is-eq contract-caller DAO_CONTRACT) (is-administrator tx-sender)) (err ERR_UNAUTHORIZED))
    ...
    (let (
      ...
      (min-reserves (/ (* circulating-supply (var-get reserve-ratio)) u100))
      (remaining-reserves (- (var-get strategic-reserves) amount))
    )
      (asserts! (>= remaining-reserves min-reserves) (err ERR_BELOW_THRESHOLD))
      ...
      (let (
        ...
        ;; Update treasury tracking
        (try! (contract-call? TOKENOMICS_CONTRACT update-strategic-reserves amount u2))
        (var-set strategic-reserves remaining-reserves)
        (var-set total-assets (- (var-get total-assets) amount))
...
(define-public (execute-investment (recipient principal) (amount uint) (proposal-id uint))
  (begin
    ;; Only the DAO contract can execute operations
    (asserts! (or (is-eq contract-caller DAO_CONTRACT) (is-administrator tx-sender)) (err ERR_UNAUTHORIZED))
...
(define-public (update-treasury-parameters (new-reserve-ratio uint) (new-pol-ratio uint))
  (begin
    ...
    ;; Validate parameters
    (asserts! (and (>= new-reserve-ratio RESERVE_RATIO_MIN) (<= new-reserve-ratio u30)) (err ERR_INVALID_PARAMETER))
...
(define-public (activate-emergency)
  (begin
    ...
    (if (>= (var-get emergency-signers) (var-get emergency-council-size))
        (begin
...
(define-read-only (get-treasury-status)
  (let (
    (circulating-supply (unwrap-panic (contract-call? TOKEN_CONTRACT get-circulating-supply)))
    (min-reserves (/ (* circulating-supply (var-get reserve-ratio)) u100))
    (target-pol (/ (* circulating-supply (var-get pol-ratio)) u100))
  )
    {
...
(define-read-only (check-reserve-ratio)
  (let (
    ...
    (>= (var-get strategic-reserves) min-reserves)
  ))
...
(define-read-only (check-pol-ratio)
  (let (
    ...
    (>= (var-get protocol-owned-liquidity) target-pol)
  ))
...
(define-public (update-emergency-council-size (new-size uint))
  (begin
    (asserts! (is-administrator tx-sender) (err ERR_UNAUTHORIZED))
    (asserts! (and (>= new-size u2) (<= new-size u7)) (err ERR_INVALID_PARAMETER))
    (var-set emergency-council-size new-size)
    (ok true)))
...
(define-public (initialize-treasury-tracking)
  (begin
    ...
    (let (
      (treasury-balance (unwrap-panic (contract-call? TOKEN_CONTRACT get-balance TREASURY_PRINCIPAL)))
      (liquidity-balance (unwrap-panic (contract-call? TOKEN_CONTRACT get-balance LIQUIDITY_PRINCIPAL)))
      (reserves-balance (unwrap-panic (contract-call? TOKEN_CONTRACT get-balance STRATEGIC_RESERVES_PRINCIPAL)))
    )
      ;; Initialize tracking values
      (var-set strategic-reserves (+ treasury-balance reserves-balance))
      (var-set protocol-owned-liquidity liquidity-balance)
      (var-set total-assets (+ (+ treasury-balance reserves-balance) liquidity-balance))
