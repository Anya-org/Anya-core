...
(define-constant ERR_NOT_FOUND u404;);(define-constant ERR_ALREADY_EXISTS u405;);
(define-constant ERR_ALLOCATION_LIMIT_EXCEEDED u406;);(define-constant ERR_BELOW_MIN_THRESHOLD u407;);
(define-constant ERR_EMERGENCY_ACTIVE u408;);(define-constant ERR_TRANSACTION_FAILED u409;);
...
(define-data-var last-rebalance-block uint u0;);(define-data-var rebalance-interval uint u14400;) ;; ~100 days with 10-min blocks
(define-data-var emergency-active bool false;);
(define-data-var total-treasury-value uint u0;);(define-data-var min-reserve-ratio uint u15;) ;; 15% in protocol-native assets
...
(define-constant RISK_LEVEL_MEDIUM u2;);(define-constant RISK_LEVEL_HIGH u3;)
...
(define-public (register-asset 
               (symbol (string-ascii 10;););
               (name (string-ascii 64;););               (asset-type uint;);               (risk-level uint;);               (target-allocation uint;);               (price-in-usd uint;);               (balance uint;);               (decimal-places uint;);               (contract-address (optional principal;);););  (begin
    ...
    ;; Validate asset type
    (asserts! (and (>= asset-type ASSET_TYPE_NATIVE;) (<= asset-type ASSET_TYPE_OTHER;);) ;              (err ERR_INVALID_PARAMETER;);)
    
    ;; Validate risk level
    (asserts! (and (>= risk-level RISK_LEVEL_LOW;) (<= risk-level RISK_LEVEL_HIGH;);) ;              (err ERR_INVALID_PARAMETER;);)
    ...
    (asserts! (<= target-allocation (var-get max-single-asset-allocation;);) ;              (err ERR_ALLOCATION_LIMIT_EXCEEDED;);)
...
(define-public (update-asset 
               (symbol (string-ascii 10;););
               (target-allocation (optional uint;););               (price-in-usd (optional uint;););               (balance (optional uint;););               (enabled (optional bool;);););  (begin
    ;; Check administrator authorization
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;);)
...
;)
      ...
      (when (is-some target-allocation;);        (asserts! (<= (default-to u0 target-allocation;) (var-get max-single-asset-allocation;);) ;                  (err ERR_ALLOCATION_LIMIT_EXCEEDED;););)
...
(define-public (execute-rebalance;);
  (begin
    ...
    (asserts! (>= (- block-height (var-get last-rebalance-block;);) (var-get rebalance-interval;);) ;              (err ERR_INVALID_PARAMETER;);)
    ...
    (let (
      (rebalance-plan (calculate-rebalance-plan;););      (operation-id (+ (var-get operation-counter;) u1;););)
...
(define-public (create-allocation-snapshot;);
  (begin
    ...
    (let (
      (allocation-data (calculate-allocation-data;););      (snapshot-id (+ (var-get allocation-snapshot-counter;) u1;););      (target-allocation (unwrap! (map-get? allocation-targets {strategy: (var-get diversification-strategy;)};) ;                                 (err ERR_NOT_FOUND;);););      (rebalance-required (or
                            (> (abs-diff (get native-percent allocation-data;) (get native-percent target-allocation;);) u500;);                            (> (abs-diff (get stable-percent allocation-data;) (get stable-percent target-allocation;);) u500;);                            (> (abs-diff (get btc-percent allocation-data;) (get btc-percent target-allocation;);) u500;);                            (> (abs-diff (get defi-percent allocation-data;) (get defi-percent target-allocation;);) u500;);                            (> (abs-diff (get other-percent allocation-data;) (get other-percent target-allocation;);) u500;)
;););)
...
(define-public (change-strategy (new-strategy (string-ascii 20;);););
  (begin
    ...
    ;; Validate strategy
    (asserts! (is-some (map-get? allocation-targets {strategy: new-strategy};);) ;              (err ERR_INVALID_PARAMETER;);)
...
(define-public (execute-targeted-rebalance (assets (list 10 (string-ascii 10;););););
  (begin
    ...
    (let (
      (rebalance-plan (calculate-targeted-rebalance-plan assets;););      (operation-id (+ (var-get operation-counter;) u1;););)
      ;; Execute the rebalance plan
      (try! (execute-rebalance-plan rebalance-plan;);)
...
(define-read-only (get-current-allocation;);
  (let ((snapshot-id (var-get allocation-snapshot-counter;);););    (map-get? allocation-tracking snapshot-id;););)
...
(define-private (calculate-allocation-data;);
  (let (
    (assets (get-all-asset-symbols;););    (total-value u0;);    (native-value u0;);    (stable-value u0;);    (btc-value u0;);    (defi-value u0;);    (other-value u0;)
;)
...
(define-public (add-signer (signer principal;););
  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;););    (map-set signers signer true;);    (ok true;););)

;; Remove a signer
(define-public (remove-signer (signer principal;););
  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;););    (map-set signers signer false;);    (ok true;););)

;; Toggle diversification
(define-public (toggle-diversification (enabled bool;););
  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;););    (var-set diversification-enabled enabled;);    (ok true;););)

;; Toggle allocation tracking
(define-public (toggle-allocation-tracking (enabled bool;););
  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;););    (var-set allocation-tracking-enabled enabled;);    (ok true;););)

;; Toggle emergency mode
(define-public (toggle-emergency-mode (enabled bool;););
  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;););    (var-set emergency-active enabled;);    (ok true;););)

;; Update diversification parameters
(define-public (update-diversification-parameters
               (new-rebalance-interval (optional uint;););
               (new-min-reserve-ratio (optional uint;););               (new-max-single-asset-allocation (optional uint;);););  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;);)
