...
(define-constant ERR_INVALID_PROPOSAL u402)
(define-constant ERR_PROPOSAL_EXISTS u403)
(define-constant ERR_INSUFFICIENT_BALANCE u404)
(define-constant ERR_VOTING_PERIOD_EXPIRED u405)
(define-constant ERR_ALREADY_VOTED u406)
(define-constant ERR_PROPOSAL_NOT_APPROVED u407)
(define-constant ERR_TIMELOCK_NOT_EXPIRED u408)
(define-constant ERR_INVALID_PARAMETER u409)
...
;; Governance Parameters (Adjustable via proposals)
(define-data-var proposal-submission-threshold uint u10000000) ;; 100 AGT (with 8 decimals)
(define-data-var voting-period-blocks uint u10080) ;; ~7 days with 10-min blocks
...
(define-constant PROPOSAL_TYPE_TREASURY_OPERATION u2)
(define-constant PROPOSAL_TYPE_EMISSION_ADJUSTMENT u3)
(define-constant PROPOSAL_TYPE_COMMUNITY_GRANT u4)
(define-constant PROPOSAL_TYPE_SYSTEM_UPGRADE u5)

;; Proposal Status
(define-constant PROPOSAL_STATUS_ACTIVE u1)
(define-constant PROPOSAL_STATUS_APPROVED u2)
(define-constant PROPOSAL_STATUS_REJECTED u3)
(define-constant PROPOSAL_STATUS_EXECUTED u4)
(define-constant PROPOSAL_STATUS_CANCELLED u5)

;; Voting Option
(define-constant VOTE_OPTION_YES u1)
(define-constant VOTE_OPTION_NO u2)
(define-constant VOTE_OPTION_ABSTAIN u3)

;; Treasury Operation Types
(define-constant TREASURY_OP_BUYBACK u1)
(define-constant TREASURY_OP_ADD_LIQUIDITY u2)
(define-constant TREASURY_OP_REMOVE_LIQUIDITY u3)
(define-constant TREASURY_OP_GRANT u4)
(define-constant TREASURY_OP_INVESTMENT u5)
...
(define-public (submit-proposal
  ...
  (let
    (
      ...
      (token-balance (unwrap-panic (contract-call? TOKEN_CONTRACT get-balance tx-sender)))
    )
...
;; Vote on a proposal
(define-public (vote (proposal-id uint) (vote-option uint) (amount uint))
  (let
    (
...
(define-public (execute-proposal (proposal-id uint))
  (let
    (
      ...
      (circulating-supply (unwrap-panic (contract-call? TOKEN_CONTRACT get-circulating-supply)))
      (quorum-requirement (/ (* circulating-supply (var-get quorum-percentage)) u100))
    )
    ...
    (match (get proposal-type proposal)
      ...
      PROPOSAL_TYPE_SYSTEM_UPGRADE (execute-system-upgrade proposal-id proposal)
...
(define-public (finalize-voting (proposal-id uint))
  (let
    (
      ...
      (circulating-supply (unwrap-panic (contract-call? TOKEN_CONTRACT get-circulating-supply)))
      (quorum-requirement (/ (* circulating-supply (var-get quorum-percentage)) u100))
      (approval-requirement (/ (* total-votes (var-get approval-threshold)) u100))
    )
    ;; Check proposal is active
    (asserts! (is-eq (get status proposal) PROPOSAL_STATUS_ACTIVE) (err ERR_INVALID_PROPOSAL))
...
(define-public (execute-buyback-and-burn (amount uint))
  (begin
    ;; Only the contract itself can call this (via execute-proposal)
...
(define-read-only (get-proposal-counts)
  (let
    (
      (total (var-get proposal-id-counter))
      (active-count (count-proposals-by-status PROPOSAL_STATUS_ACTIVE))
      (approved-count (count-proposals-by-status PROPOSAL_STATUS_APPROVED))
      (rejected-count (count-proposals-by-status PROPOSAL_STATUS_REJECTED))
      (executed-count (count-proposals-by-status PROPOSAL_STATUS_EXECUTED))
    )
    {
      total: total,
      active: active-count,
...
(define-private (valid-proposal-type (proposal-type uint))
  (or
    ...
    (is-eq proposal-type PROPOSAL_TYPE_TREASURY_OPERATION)
    (is-eq proposal-type PROPOSAL_TYPE_EMISSION_ADJUSTMENT)
    (is-eq proposal-type PROPOSAL_TYPE_COMMUNITY_GRANT)
    (is-eq proposal-type PROPOSAL_TYPE_SYSTEM_UPGRADE)
  ))

;; Check if a vote option is valid
(define-private (valid-vote-option (vote-option uint))
  (or
    (is-eq vote-option VOTE_OPTION_YES)
    (is-eq vote-option VOTE_OPTION_NO)
    (is-eq vote-option VOTE_OPTION_ABSTAIN)
  ))
...
(define-private (execute-treasury-operation (proposal-id uint) (proposal {
  ...
  (let
    (
      (treasury-data (unwrap! (get action-data proposal) (err ERR_INVALID_PARAMETER)))
      (op-type (buff-to-uint-be (unwrap! (slice? treasury-data u0 u1) (err ERR_INVALID_PARAMETER))))
      (amount (buff-to-uint-be (unwrap! (slice? treasury-data u1 u9) (err ERR_INVALID_PARAMETER))))
    )
    (match op-type
...
(define-private (execute-emission-adjustment (proposal-id uint) (proposal {
  ...
  (let
    (
      (emission-data (unwrap! (get action-data proposal) (err ERR_INVALID_PARAMETER)))
      (new-halving-interval (buff-to-uint-be (unwrap! (slice? emission-data u0 u8) (err ERR_INVALID_PARAMETER))))
    )
    (adjust-emission-parameters new-halving-interval)
  ))
...
(define-public (update-governance-parameters
               ...
               (new-timelock-period (optional uint))
