...
(define-constant ERR_INVALID_PARAMETER u407)
...
;; Voting parameters (updatable)
(define-data-var proposal-threshold uint u10000000) ;; 100 AGT (with 8 decimals)
(define-data-var voting-period uint u10080) ;; ~7 days with 10-min blocks
...
(define-constant VOTE_TYPE_QUADRATIC u2)
(define-constant VOTE_TYPE_CONVICTION u3)
...
(define-constant PROPOSAL_TYPE_EMISSION u3)
(define-constant PROPOSAL_TYPE_VESTING u4)
...
(define-public (create-proposal
  ...
  (let
    ...
    (asserts! (and (>= proposal-type u1) (<= proposal-type u5)) (err ERR_INVALID_PARAMETER))
    (asserts! (and (>= vote-type u1) (<= vote-type u3)) (err ERR_INVALID_PARAMETER))
    
    ;; Determine threshold based on proposal type
    (let
      (
        (required-threshold (get-threshold-for-type proposal-type))
...
(define-public (vote (proposal-id uint) (vote-option uint) (amount uint))
  (let
    ...
    (let
      ...
      (if (is-eq (get vote-type proposal) VOTE_TYPE_QUADRATIC)
          (let
            (
  ...
  (let
    ...
    (let
      (
        ...
        (quorum-requirement (get quorum-requirement proposal))
      )
      ...
      (if (>= total-votes quorum-requirement)
          ...
          (if (>= (/ (* yes-votes u100) (+ yes-votes no-votes)) required-threshold)
              ;; Proposal passed
              (begin
                (map-set proposals proposal-id (merge proposal { status: PROPOSAL_STATUS_PASSED }))
                (ok true)
              )
              ;; Proposal rejected
              (begin
...
(define-public (execute-proposal (proposal-id uint))
  (let
    ...
    (if (and (is-some (get action-contract proposal)) 
             (is-some (get action-function proposal)))
        (begin
...
(define-read-only (get-threshold-for-type (proposal-type uint))
  (match proposal-type
    ...
    PROPOSAL_TYPE_GOVERNANCE (var-get supermajority-threshold)
    (var-get approval-threshold) ;; Default
  ))
...
(define-read-only (get-proposal-stats (proposal-id uint))
  (let
    ...
    (ok {
      ...
      quorum-percentage: (if (> (get quorum-requirement proposal) u0) 
                              (/ (* total-votes u100) (get quorum-requirement proposal)) 
                              u0),
      required-threshold: (get required-threshold proposal),
...
;; Calculate vote weight based on vote type
(define-private (calculate-vote-weight (vote-type uint) (amount uint))
  (match vote-type
...
(define-public (update-voting-parameters
              ...
              (new-supermajority-threshold (optional uint)))
  (begin
    ...
    (match new-quorum-percentage
      value (asserts! (and (> value u0) (<= value u100)) (err ERR_INVALID_PARAMETER))
...
(define-public (update-operation-thresholds
              ...
              (new-emission-threshold (optional uint))
              (new-vesting-threshold (optional uint)))
  (begin
    ;; Only admins can update parameters
    (asserts! (is-administrator tx-sender) (err ERR_UNAUTHORIZED))
