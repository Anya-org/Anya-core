...
(define-constant ERR_COOLDOWN_ACTIVE u405)
(define-constant ERR_BELOW_THRESHOLD u406)
(define-constant ERR_ABOVE_THRESHOLD u407)
(define-constant ERR_DEX_INTEGRATION_FAILED u408)
(define-constant ERR_MAX_SLIPPAGE_EXCEEDED u409)
(define-constant ERR_MAX_IMPACT_EXCEEDED u410)
(define-constant ERR_EXECUTION_FAILED u411)
(define-constant ERR_EMERGENCY_MODE_ACTIVE u412)
(define-constant ERR_STRATEGY_NOT_FOUND u413)
(define-constant ERR_MARKET_CONDITIONS_UNMET u414)
(define-constant ERR_PRICE_ORACLE_UNAVAILABLE u415)
...
(define-constant DEX_ADAPTER 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.dex-adapter)
(define-constant OPERATIONS_CONTRACT 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.operations-manager)
(define-constant ML_GOVERNANCE 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.ml-governance)

;; Data Variables
(define-data-var buyback-enabled bool true)
(define-data-var emergency-mode bool false)
(define-data-var min-cooldown-blocks uint u14400) ;; ~100 days with 10-min blocks
...
(define-public (execute-buyback (strategy-id uint) (stx-amount uint))
  (begin
    ...
    (asserts! (>= (- block-height (var-get last-buyback-block)) (var-get min-cooldown-blocks))
    ...
    (let ((strategy (unwrap! (map-get? buyback-strategies strategy-id) (err ERR_STRATEGY_NOT_FOUND))))
      ...
      (asserts! (>= (- block-height (get last-executed strategy)) (get min-blocks-between strategy))
               (err ERR_COOLDOWN_ACTIVE))
      
      ;; Check if amount is within limits
      (let (
        (max-buyback-amount (/ (* (var-get treasury-stx-balance) (var-get max-buyback-percentage)) u10000))
        (strategy-max-amount (/ (* (var-get treasury-stx-balance) (get max-size-bps strategy)) u10000))
        (effective-max-amount (if (< max-buyback-amount strategy-max-amount) 
                                max-buyback-amount 
                                strategy-max-amount))
      )
        (asserts! (<= stx-amount effective-max-amount) (err ERR_ABOVE_THRESHOLD))
        (asserts! (>= stx-amount u100000000) (err ERR_BELOW_THRESHOLD)) ;; Minimum 1 STX
        ...
        (let (
          (token-price (unwrap! (contract-call? DEX_ADAPTER get-token-price) (err ERR_PRICE_ORACLE_UNAVAILABLE)))
          (expected-token-amount (/ (* stx-amount u100000000) token-price))
          (min-token-amount (/ (* expected-token-amount (- u10000 (var-get max-slippage-bps))) u10000))
        )
          ;; Execute the swap
          (let (
            (swap-result (unwrap! (contract-call? DEX_ADAPTER swap-stx-for-tokens stx-amount 
                                               min-token-amount (var-get max-price-impact-bps))
                                 (err ERR_DEX_INTEGRATION_FAILED)))
            (tokens-received (get token-amount swap-result))
            (effective-price (get effective-price swap-result))
            (price-impact-bps (get price-impact-bps swap-result))
          )
            ;; Calculate tokens to burn and tokens to treasury
            (let (
              (burn-percentage (get-burn-percentage (get strategy-type strategy)))
              (tokens-to-burn (/ (* tokens-received burn-percentage) u10000))
              (tokens-to-treasury (- tokens-received tokens-to-burn))
              (new-operation-id (+ (var-get buyback-counter) u1))
            )
              ;; Burn tokens
              (when (> tokens-to-burn u0)
                (try! (contract-call? TOKEN_CONTRACT burn tokens-to-burn tx-sender))
                (var-set total-token-burned (+ (var-get total-token-burned) tokens-to-burn))
              )
              
              ;; Send tokens to treasury
              (when (> tokens-to-treasury u0)
                (try! (contract-call? TOKEN_CONTRACT transfer tokens-to-treasury tx-sender TREASURY_CONTRACT none))
              )
              ...
              (map-set buyback-operations new-operation-id
                {
                  ...
                  ml-recommendation-id: (if (is-eq (get strategy-type strategy) STRATEGY_ML_RECOMMENDED)
                                         (some (get-ml-recommendation-id))
                                         none)
                }
              )
              ...
              ;; Update global stats
              (var-set buyback-counter new-operation-id)
              (var-set last-buyback-block block-height)
              (var-set total-value-deployed (+ (var-get total-value-deployed) stx-amount))
              (var-set treasury-stx-balance (- (var-get treasury-stx-balance) stx-amount))
...
(define-public (update-market-conditions
  ...
  (begin
    ...
    (let (
      (ma-ratio (if (> ma-50-blocks u0)
                  (/ (* current-price u10000) ma-50-blocks)
                  u10000))
    )
      ;; Update market conditions
...
(define-public (update-treasury-metrics (stx-balance uint))
  (begin
    ...
    (var-set treasury-stx-balance stx-balance)
    (var-set last-treasury-metric-update block-height)
...
(define-read-only (simulate-buyback (strategy-id uint) (stx-amount uint))
  (let (
    (strategy (map-get? buyback-strategies strategy-id))
    (token-price (contract-call? DEX_ADAPTER get-token-price))
  )
    (match strategy
      s (match token-price
          price (let (
            (expected-token-amount (/ (* stx-amount u100000000) price))
            (price-impact (contract-call? DEX_ADAPTER get-price-impact stx-amount))
            (burn-percentage (get-burn-percentage (get strategy-type s)))
          )
            (match price-impact
              impact (if (> impact (var-get max-price-impact-bps))
                ...
                (ok {
                  can-execute: (and 
                                 (get enabled s)
                                 (>= (- block-height (var-get last-buyback-block)) (var-get min-cooldown-blocks))
                                 (>= (- block-height (get last-executed s)) (get min-blocks-between s))
                                 (<= stx-amount (/ (* (var-get treasury-stx-balance) (var-get max-buyback-percentage)) u10000))
                                 (check-market-conditions (get strategy-type s))
                                ),
              ...
              (err ERR_PRICE_ORACLE_UNAVAILABLE)))
          (err ERR_PRICE_ORACLE_UNAVAILABLE))
...
(define-private (calculate-buy-pressure (price-change-bps int) (volume uint))
  (some u5000)) ;; Simplified placeholder

;; Calculate sell pressure from market metrics
(define-private (calculate-sell-pressure (price-change-bps int) (volume uint))
  (some u5000)) ;; Simplified placeholder

;; Check market conditions based on strategy type
(define-private (check-market-conditions (strategy-type uint))
  (let (
    (conditions (var-get market-conditions))
  )
    (if (is-eq strategy-type STRATEGY_COUNTER_CYCLICAL)
        ;; For counter-cyclical: check if price is below threshold compared to MA
        (<= (get ma-50-blocks-ratio conditions) 
           (- u10000 (var-get min-price-drop-trigger-bps)))
...
;; Get ML recommendation ID (placeholder)
(define-private (get-ml-recommendation-id)
  u1)
...
(define-public (add-buyback-executor (executor principal))
  (begin
    (asserts! (is-administrator tx-sender) (err ERR_UNAUTHORIZED))
    (map-set buyback-executors executor true)
    (ok true)))
...
(define-public (remove-buyback-executor (executor principal))
  (begin
    (asserts! (is-administrator tx-sender) (err ERR_UNAUTHORIZED))
    (map-set buyback-executors executor false)
    (ok true)))
...
(define-public (add-or-update-strategy
  ...
  (begin
    ...
    (asserts! (and (>= strategy-type STRATEGY_COUNTER_CYCLICAL) (<= strategy-type STRATEGY_ML_RECOMMENDED))
    ...
    (let (
      (existing-strategy (map-get? buyback-strategies strategy-id))
      (old-operations (default-to u0 (get total-operations (default-to 
                                                          {total-operations: u0} 
                                                          existing-strategy))))
      (old-value (default-to u0 (get total-value-deployed (default-to 
                                                          {total-value-deployed: u0} 
                                                          existing-strategy))))
      (old-tokens (default-to u0 (get total-tokens-purchased (default-to 
                                                          {total-tokens-purchased: u0} 
                                                          existing-strategy))))
      (old-burned (default-to u0 (get total-tokens-burned (default-to 
                                                          {total-tokens-burned: u0} 
                                                          existing-strategy))))
      (old-executed (default-to u0 (get last-executed (default-to 
                                                    {last-executed: u0} 
                                                    existing-strategy))))
    )
...
(define-public (update-buyback-parameters
    ...
    (max-percentage (optional uint))
    (max-slippage (optional uint))
    (max-impact (optional uint))
    (target-ratio (optional uint))
    (burn-percentage (optional uint))
    (price-drop-trigger (optional uint)))
  (begin
    (asserts! (is-administrator tx-sender) (err ERR_UNAUTHORIZED))
