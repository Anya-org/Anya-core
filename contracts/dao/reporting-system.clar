...
(define-constant REPORT_TYPE_TOKENOMICS u2;);(define-constant REPORT_TYPE_GOVERNANCE u3;);
(define-constant REPORT_TYPE_OPERATIONS u4;)

;; Report Frequencies
(define-constant FREQUENCY_DAILY u1;);
(define-constant FREQUENCY_WEEKLY u7;);(define-constant FREQUENCY_BIWEEKLY u14;);
(define-constant FREQUENCY_MONTHLY u30;);(define-constant FREQUENCY_QUARTERLY u90;)
...
;; Report data (metrics snapshots;);(define-map report-data
...
;; Generate a report
(define-public (generate-report (report-type uint;); (is-public bool;) (metrics-to-include (list 10 (string-ascii 64;););););  (let (
    (report-type-info (unwrap! (map-get? report-types {report-type: report-type};) (err ERR_INVALID_PARAMETER;);););    (report-id (+ (var-get report-counter;) u1;););)
    ...
    (let (
      (collected-metrics (collect-metrics-for-report report-type metrics-to-include;););      (data-hash (hash collected-metrics;););      (metrics-list (if (> (len metrics-to-include;) u0;)
                       metrics-to-include
...
;; Automated report generation (called by a scheduler;);(define-public (generate-scheduled-reports;);
  (begin
    ;; Check report generator authorization
    (asserts! (is-report-generator tx-sender;) (err ERR_UNAUTHORIZED;);)
    ...
    (let (
      (treasury-result (generate-report-if-enabled REPORT_TYPE_TREASURY true (list;);););      (tokenomics-result (generate-report-if-enabled REPORT_TYPE_TOKENOMICS true (list;);););      (governance-result (generate-report-if-enabled REPORT_TYPE_GOVERNANCE true (list;);););      (operations-result (generate-report-if-enabled REPORT_TYPE_OPERATIONS true (list;););)
;);      (ok (var-get report-counter;););)
;);)

;; Helper function to generate a report if its type is enabled
(define-private (generate-report-if-enabled (report-type uint;); (is-public bool;) (metrics (list 10 (string-ascii 64;););););  (match (map-get? report-types {report-type: report-type};)
    type-info (if (get is-enabled type-info;);                 (generate-report report-type is-public metrics;);                 (ok u0;););    (ok u0;)
;)
;)
...
(define-public (subscribe-to-report (report-type uint;););
  (begin
    ...
    (match (map-get? report-types {report-type: report-type};)
      type-info (if (get is-enabled type-info;);                    (ok true;);                    (err ERR_REPORT_DISABLED;););      (err ERR_INVALID_PARAMETER;)
;)
;);)
...
(define-public (get-report-details (report-id uint;););
  (match (map-get? reports report-id;)
    report (if (or (get is-public report;) (is-report-generator tx-sender;););                (ok report;);                (err ERR_UNAUTHORIZED;););    (err ERR_REPORT_NOT_FOUND;)
;);)

;; Get report metrics
(define-public (get-report-metrics (report-id uint;););
  (match (map-get? reports report-id;)
    report (if (or (get is-public report;) (is-report-generator tx-sender;););                (ok (get-metrics-for-report report-id (get metrics-included report;);););                (err ERR_UNAUTHORIZED;););    (err ERR_REPORT_NOT_FOUND;)
;);)

;; Helper Functions

;; Collect metrics for a report
(define-private (collect-metrics-for-report (report-type uint;); (metrics-to-include (list 10 (string-ascii 64;););););  (match report-type
...
(define-private (collect-treasury-metrics (metrics-to-include (list 10 (string-ascii 64;););););
  (concat 
    (concat 
      (unwrap-panic (contract-call? METRICS_CONTRACT get-treasury-metric "reserves_ratio";););      (unwrap-panic (contract-call? METRICS_CONTRACT get-treasury-metric "pol_percentage";);););    (concat 
      (unwrap-panic (contract-call? METRICS_CONTRACT get-treasury-metric "protocol_revenue";););      (unwrap-panic (contract-call? METRICS_CONTRACT get-treasury-metric "runway_months";););)
;)
;)
...
(define-private (collect-tokenomics-metrics (metrics-to-include (list 10 (string-ascii 64;););););
  (concat 
    (concat 
      (unwrap-panic (contract-call? METRICS_CONTRACT get-token-metric "circulating_supply";););      (unwrap-panic (contract-call? METRICS_CONTRACT get-token-metric "velocity";);););    (concat 
      (unwrap-panic (contract-call? METRICS_CONTRACT get-token-metric "active_wallets";););      (unwrap-panic (contract-call? METRICS_CONTRACT get-token-metric "holder_distribution";););)
;)
;)
...
(define-private (collect-governance-metrics (metrics-to-include (list 10 (string-ascii 64;););););
  (concat 
    (concat 
      (unwrap-panic (contract-call? METRICS_CONTRACT get-governance-metric "proposal_volume";););      (unwrap-panic (contract-call? METRICS_CONTRACT get-governance-metric "voting_participation";);););    (concat 
      (unwrap-panic (contract-call? METRICS_CONTRACT get-governance-metric "execution_success_rate";););      (unwrap-panic (contract-call? METRICS_CONTRACT get-governance-metric "voter_retention";););)
;)
;)
...
(define-private (add-report-metric (metric-name (string-ascii 64;);); (report-id uint;););  (let (
    (metric-value (get-metric-value metric-name;);););    (map-set report-data
...
(define-private (get-metric-value (metric-name (string-ascii 64;);););
  (let (
    (token-metric (contract-call? METRICS_CONTRACT get-token-metric metric-name;););    (treasury-metric (contract-call? METRICS_CONTRACT get-treasury-metric metric-name;););    (governance-metric (contract-call? METRICS_CONTRACT get-governance-metric metric-name;););    (market-metric (contract-call? METRICS_CONTRACT get-market-metric metric-name;);););    (if (is-ok token-metric;);        (get value (unwrap-panic token-metric;););        (if (is-ok treasury-metric;);            (get value (unwrap-panic treasury-metric;););            (if (is-ok governance-metric;);                (get value (unwrap-panic governance-metric;););                (if (is-ok market-metric;);                    (get value (unwrap-panic market-metric;);)
...
(define-private (get-metrics-for-report (report-id uint;); (metrics (list 10 (string-ascii 64;););););  (map (lambda (metric-name;) 
          {
            name: metric-name, 
            data: (default-to 
...
(define-private (generate-report-uri (report-id uint;); (report-type uint;););  (let (
    (type-name (match report-type
                 ...
                 "unknown";);););    (concat 
      (concat "https://dao.anya.ai/reports/" type-name "/";);      (concat (uint-to-string report-id;) ".json";)
;)
;)
;)

;; Convert uint to string (simple implementation;);(define-private (uint-to-string (value uint;););
  (match value
    u0 "0"
    u1 "1"
    u2 "2"
...
;; Compute a hash of data (placeholder implementation;);(define-private (hash (data (string-ascii 64;););)
...
(define-read-only (get-available-report-types;);
  (let (
    (treasury (map-get? report-types {report-type: REPORT_TYPE_TREASURY};););    (tokenomics (map-get? report-types {report-type: REPORT_TYPE_TOKENOMICS};););    (governance (map-get? report-types {report-type: REPORT_TYPE_GOVERNANCE};););    (operations (map-get? report-types {report-type: REPORT_TYPE_OPERATIONS};);););    (filter remove-disabled (list
      (some {
...
(define-read-only (get-recent-reports (limit uint;););
  (let (
    (current-id (var-get report-counter;););    (start-id (if (> current-id limit;) (- current-id limit;) u1;);););    (get-reports-in-range start-id current-id;)
;)
;)

;; Get reports in a specific range
(define-read-only (get-reports-in-range (start-id uint;); (end-id uint;););  (map get-public-report-or-none (sequence start-id end-id;););)

;; Helper function for report retrieval
(define-private (get-public-report-or-none (id uint;););
  (match (map-get? reports id;)
    report (if (get is-public report;);               (some report;)
               none;)
    none;)
;)
