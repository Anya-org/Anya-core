...
(define-constant ERR_INVALID_PARAMETER u402;);(define-constant ERR_NOT_FOUND u404;);
...
(define-constant LIQUIDITY_PRINCIPAL 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.liquidity;);(define-constant TEAM_PRINCIPAL 'ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG.team;);
(define-constant COMMUNITY_PRINCIPAL 'ST2JHG361ZXG51QTKY2NQCVBPPRRE2KZB1HR05NNC.community;);(define-constant PARTNERS_PRINCIPAL 'ST2NEB84ASENDXKYGJPQW86YXQCEFEX2ZQPG87ND.partners;)
...
;; Team member allocations (for 4-year vesting with 1-year cliff;);(define-map team-allocations
  principal
  {
...
(define-public (initialize-vesting;);
  (begin
    ...
    (let (
      ...
      (treasury-amount (/ (* total-supply TREASURY_PERCENTAGE;) u100;););      (liquidity-amount (/ (* total-supply LIQUIDITY_PERCENTAGE;) u100;););      (team-amount (/ (* total-supply TEAM_PERCENTAGE;) u100;););      (community-amount (/ (* total-supply COMMUNITY_PERCENTAGE;) u100;););      (partners-amount (/ (* total-supply PARTNERS_PERCENTAGE;) u100;););)
      ;; Initialize Treasury vesting (20% initial, 48 month linear;);      (map-set allocation-vesting
        TREASURY_PRINCIPAL
        {
      ...
      ;; Initialize Liquidity vesting (50% initial, 18 month linear;);      (map-set allocation-vesting
        LIQUIDITY_PRINCIPAL
        {
          vesting-type: VESTING_TYPE_LINEAR,
      ...
      ;; Initialize Team vesting (0% initial, 12 month cliff, 36 month linear;);      (map-set allocation-vesting
        TEAM_PRINCIPAL
        {
      ...
      ;; Initialize Community vesting (10% initial, 48 month linear;);      (map-set allocation-vesting
        COMMUNITY_PRINCIPAL
        {
          vesting-type: VESTING_TYPE_LINEAR,
      ...
      ;; Initialize Partners vesting (10% initial, 36 month linear;);      (map-set allocation-vesting
        PARTNERS_PRINCIPAL
        {
          vesting-type: VESTING_TYPE_LINEAR,
...
(define-public (process-allocation-vesting (allocation-principal principal;););
  (begin
    ...
    (let (
      (vesting-data (unwrap! (map-get? allocation-vesting allocation-principal;) (err ERR_NOT_FOUND;);););      (blocks-since-launch (- block-height (var-get launch-timestamp;);););      (months-since-launch (/ blocks-since-launch (var-get blocks-per-month;););)
;)
      ;; Calculate vested amount based on vesting type
      (if (is-eq (get vesting-type vesting-data;) VESTING_TYPE_CLIFF;);          (process-cliff-vesting allocation-principal vesting-data months-since-launch;);          (process-linear-vesting allocation-principal vesting-data months-since-launch;););)
;);)
...
(define-public (process-all-vestings;);
  (begin
    ...
    (try! (process-allocation-vesting TREASURY_PRINCIPAL;););    (try! (process-allocation-vesting LIQUIDITY_PRINCIPAL;););    (try! (process-allocation-vesting TEAM_PRINCIPAL;););    (try! (process-allocation-vesting COMMUNITY_PRINCIPAL;););    (try! (process-allocation-vesting PARTNERS_PRINCIPAL;);)
...
(define-public (initialize-team-member (member principal;); (allocation-percentage uint;););  (begin
    ...
    ;; Validate percentage (must be between 5% and 40%;);    (asserts! (and (>= allocation-percentage u5;) (<= allocation-percentage u40;);) (err ERR_INVALID_PARAMETER;);)
    
    ;; Get team allocation data
    (let (
      (team-data (unwrap! (map-get? allocation-vesting TEAM_PRINCIPAL;) (err ERR_NOT_FOUND;);););      (team-total (get total-allocation team-data;););      (member-amount (/ (* team-total allocation-percentage;) u100;););)
...
(define-public (process-team-member-vesting (member principal;););
  (begin
    ...
    (let (
      (member-data (unwrap! (map-get? team-allocations member;) (err ERR_NOT_FOUND;);););      (blocks-since-launch (- block-height (var-get launch-timestamp;);););      (months-since-launch (/ blocks-since-launch (var-get blocks-per-month;);););      (team-data (unwrap! (map-get? allocation-vesting TEAM_PRINCIPAL;) (err ERR_NOT_FOUND;););)
;)
      ...
      (let (
        (triggers (get milestone-triggers member-data;););        (percentages (get milestone-percentages member-data;););        (total-amount (get total-amount member-data;););        (already-released (get released-amount member-data;););        (newly-vested u0;)
;)
...
(define-public (add-team-member-milestone (member principal;); (trigger-month uint;) (percentage uint;););  (begin
    ...
    ;; Get team member allocation data
    (let (
      (member-data (unwrap! (map-get? team-allocations member;) (err ERR_NOT_FOUND;);););      (triggers (get milestone-triggers member-data;););      (percentages (get milestone-percentages member-data;););)
...
(define-read-only (calculate-vested-amount (allocation-principal principal;););
  (let (
    (vesting-data (unwrap! (map-get? allocation-vesting allocation-principal;) (err ERR_NOT_FOUND;);););    (blocks-since-launch (- block-height (var-get launch-timestamp;);););    (months-since-launch (/ blocks-since-launch (var-get blocks-per-month;);););    (total-allocation (get total-allocation vesting-data;););    (initial-percentage (get initial-percentage vesting-data;););    (initial-amount (/ (* total-allocation initial-percentage;) u100;););    (vesting-amount (- total-allocation initial-amount;);););    (if (is-eq (get vesting-type vesting-data;) VESTING_TYPE_CLIFF;)
        ...
        (if (< months-since-launch (get cliff-months vesting-data;);)
            ...
            (let (
              (months-after-cliff (- months-since-launch (get cliff-months vesting-data;);););              (vesting-period (get vesting-months vesting-data;););              (vested-so-far (if (>= months-after-cliff vesting-period;)
                               vesting-amount
                               (/ (* vesting-amount months-after-cliff;) vesting-period;);););              (total-vested (+ initial-amount vested-so-far;););              (progress-percentage (/ (* total-vested u100;) total-allocation;);););              (ok {
                total-allocation: total-allocation,
...
;)
        ...
        (let (
          (vested-so-far (if (>= months-since-launch (get vesting-months vesting-data;);)
                           vesting-amount
                           (/ (* vesting-amount months-since-launch;) (get vesting-months vesting-data;););););          (total-vested (+ initial-amount vested-so-far;););          (progress-percentage (/ (* total-vested u100;) total-allocation;);););          (ok {
            total-allocation: total-allocation,
            vested-amount: total-vested,
            unreleased-amount: (- total-vested (get released-amount vesting-data;);),
            vesting-progress-percentage: progress-percentage
...
(define-read-only (get-vesting-summary;);
  (ok {
    ...
    months-since-launch: (if (> (var-get launch-timestamp;) u0;) ;                              (/ (- block-height (var-get launch-timestamp;);) (var-get blocks-per-month;);)
                              u0;),
...
(define-private (process-cliff-vesting 
                ...
                (vesting-data {
                  ...
                  released-amount: uint,
                  last-release-block: uint
                };) ;                (months-since-launch uint;););  (let (
    (total-allocation (get total-allocation vesting-data;););    (initial-percentage (get initial-percentage vesting-data;););    (initial-amount (/ (* total-allocation initial-percentage;) u100;););    (vesting-amount (- total-allocation initial-amount;););    (cliff-months (get cliff-months vesting-data;););    (vesting-months (get vesting-months vesting-data;););    (released-so-far (get released-amount vesting-data;););)
    ;; Check if cliff period has passed
    (if (< months-since-launch cliff-months;)
        ...
        ;; After cliff, calculate linear vesting
        (let (
          (months-after-cliff (- months-since-launch cliff-months;););          (vested-so-far (if (>= months-after-cliff vesting-months;)
                           vesting-amount
                           (/ (* vesting-amount months-after-cliff;) vesting-months;);););          (total-vested (+ initial-amount vested-so-far;););          (to-release (- total-vested released-so-far;);););          (if (> to-release u0;);              (begin
...
(define-private (process-linear-vesting 
    ...
    (total-allocation (get total-allocation vesting-data;););    (initial-percentage (get initial-percentage vesting-data;););    (initial-amount (/ (* total-allocation initial-percentage;) u100;););    (vesting-amount (- total-allocation initial-amount;););    (vesting-months (get vesting-months vesting-data;););    (released-so-far (get released-amount vesting-data;););    (vested-so-far (if (>= months-since-launch vesting-months;)
                      vesting-amount
                      (/ (* vesting-amount months-since-launch;) vesting-months;);););    (total-vested (+ initial-amount vested-so-far;););    (to-release (- total-vested released-so-far;);););    (if (> to-release u0;);        (begin
          ;; Update allocation vesting data
          (map-set allocation-vesting
            allocation-principal
            (merge vesting-data {
...
;; Calculate newly vested amount for milestones (placeholder implementation;);(define-private (set-newly-vested (months uint;); (triggers (list 10 uint;);) (percentages (list 10 uint;);) (total uint;) (released uint;);)
...
(define-public (update-blocks-per-month (new-value uint;););
  (begin
    (asserts! (is-administrator tx-sender;) (err ERR_UNAUTHORIZED;););    (asserts! (> new-value u0;) (err ERR_INVALID_PARAMETER;););    (var-set blocks-per-month new-value;);    (ok true;););) 
