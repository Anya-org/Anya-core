...
impl Layer2Factory {
    ...
    where
        F: Fn(Box<dyn ProtocolConfig>) -> AnyaResult<Box<dyn Layer2Protocol>> + Send + Sync + 'static,
    {
    ...
    pub fn create_protocol(&self, config: Box<dyn ProtocolConfig>) -> AnyaResult<Box<dyn Layer2Protocol>> {
        ...
        if let Some(factory_fn) = factory_functions.get(protocol_type) {
            ...
            Err(AnyaError::NotImplemented(format!("Protocol type not supported: {}", protocol_type)))
        }
    }
}
