...
impl BitVMValidator {
        ...
        match self.http_client.get(&url).send().await {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    warn!(
                        "BitVM system returned non-success status: {}",
                        response.status()
                    );
                    Ok(false)
                }
            ...
            Err(e) => {
                ...
                Err(BobError::BitVMError(format!(
                    "Failed to connect to BitVM system: {}",
                    e
                )))
            }
        }
    ...
    pub async fn verify_proof(&self, proof: BitVMProof) -> Result<bool, BobError> {
        ...
        if let Some(status) = self.verified_proofs.get(&proof.id) {
            match status {
                ...
                VerificationStatus::Error(err) => {
                    return Err(BobError::BitVMError(format!(
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                BobError::BitVMError(format!("Failed to send verification request: {}", e))
            })?;
        ...
        if !response.status().is_success() {
            ...
            warn!(
                "BitVM verification failed with status {}: {}",
                response.status(),
                error_text
            );
            return Err(BobError::BitVMError(format!(
        ...
        let result: serde_json::Value = response.json().await.map_err(|e| {
            BobError::BitVMError(format!("Failed to parse verification response: {}", e))
        })?;
    ...
    pub async fn submit_proof(&self, proof: &BitVMProof) -> Result<String, BobError> {
        ...
        let result: serde_json::Value = response.json().await.map_err(|e| {
            BobError::BitVMError(format!("Failed to parse submission response: {}", e))
        })?;
        ...
        let verification_id = result
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                BobError::BitVMError("Missing 'verification_id' field in response".to_string())
            })?;
    ...
    pub async fn check_verification_status(
        &self,
        verification_id: &str,
    ) -> Result<VerificationStatus, BobError> {
        let url = format!(
            "{}/verification/{}",
            self.config.bitvm_url(),
            verification_id
        );

        // Get verification status
        let response = self.http_client.get(&url).send().await.map_err(|e| {
            BobError::BitVMError(format!("Failed to check verification status: {}", e))
        })?;
        ...
        let status = result
            ...
            .and_then(|s| s.as_str())
            .ok_or_else(|| {
                BobError::BitVMError("Missing 'status' field in response".to_string())
            })?;
