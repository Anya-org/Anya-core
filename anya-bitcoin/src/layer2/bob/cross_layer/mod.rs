...
impl CrossLayerTransactionManager {
        ...
        let bitcoin_auth = match (&config.bitcoin_rpc_user, &config.bitcoin_rpc_password) {
            ...
            _ => Auth::CookieFile(
                config
                    .bitcoin_cookie_path
                    .clone()
                    .unwrap_or_else(|| "/tmp/.cookie".to_string()),
            ),
        };
    ...
    pub async fn send_transaction(
        ...
        match transaction.source_chain.as_str() {
            "bitcoin" => {
                self.process_bitcoin_to_evm(transaction_id.clone(), transaction)
                    .await?
            }
            "evm" => {
                self.process_evm_to_bitcoin(transaction_id.clone(), transaction)
                    .await?
            }
            _ => {
                return Err(BobError::InvalidChainError(format!(
    ...
    async fn process_bitcoin_to_evm(
        ...
        match self
            .bitcoin_client
            .get_raw_transaction_info(&btc_txid_obj, None)
        {
            Ok(tx) => {
                ...
                let evm_tx_request = self
                    .create_evm_tx_request(&evm_tx)?
                    .data(evm_tx.data.clone());

                let tx_serialized = serde_json::to_value(evm_tx_request).map_err(|e| {
                    BobError::SerializationError(format!("Failed to serialize transaction: {}", e))
                })?;

                // Send the transaction
                let evm_txid: String = self
                    .evm_client
                    .request("eth_sendTransaction", &[tx_serialized])
                    .await
                    .map_err(|e| {
                        BobError::TransactionError(format!("Failed to send EVM transaction: {}", e))
                    })?;
            ...
            Err(e) => {
                ...
                if let Some(record) = records.get_mut(&transaction_id) {
                    record.status = CrossLayerTransactionStatus::Failed(format!(
                        "Failed to verify Bitcoin transaction: {}",
                        e
                    ));
                    record.last_update = chrono::Utc::now().timestamp() as u64;
                }

                Err(BobError::BitcoinError(format!(
                    "Failed to verify Bitcoin transaction: {}",
                    e
                )))
            }
    ...
    async fn process_evm_to_bitcoin(
        ...
        let receipt: Option<serde_json::Value> = self
            ...
            .request(
                ...
                &[serde_json::Value::String(evm_txid.clone())],
            )
            .await
            .map_err(|e| {
                BobError::RpcError(format!("Failed to get EVM transaction receipt: {}", e))
            })?;
        ...
        match receipt {
            Some(receipt) => {
                ...
                let status = receipt
                    ...
                    .and_then(|s| s.as_str())
                    .ok_or_else(|| {
                        BobError::ParseError("Receipt missing status field".to_string())
                    })?;
    ...
    pub async fn get_transaction_details(
        &self,
        transaction_id: &str,
    ) -> Result<TransactionRecord, BobError> {
        let records = self.transaction_records.read().await;
