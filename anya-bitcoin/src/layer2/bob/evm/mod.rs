...
impl EvmAdapter {
    ...
    pub async fn check_connection(&self) -> Result<bool, BobError> {
        ...
        match self
            .client
            .request::<String, &str>("net_version", &[])
            .await
        {
            Ok(_) => Ok(true),
            Err(e) => {
    ...
    pub async fn send_transaction(
        ...
        let tx_serialized = serde_json::to_value(tx_request).map_err(|e| {
        ...
        let tx_hash: String = self
            ...
            .await
            .map_err(|e| {
                BobError::TransactionError(format!("Failed to send transaction: {}", e))
            })?;
        ...
        let operation = || async {
            ...
            match receipt {
                Some(receipt) => {
                    ...
                    let block_number =
                        u64::from_str_radix(block_number.trim_start_matches("0x"), 16).map_err(
                            |e| {
