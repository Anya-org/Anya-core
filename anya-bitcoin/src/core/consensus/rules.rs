...
pub fn check_consensus_rules(block: &Block, prev_hash: &BlockHash) -> Result<(), ConsensusError> {
    ...
    if block.header.prev_blockhash != *prev_hash {
        return Err(ConsensusError::General("Invalid previous block reference".to_string()));
    }
    ...
    if block.header.time > current_time + 7200 { // 2 hours in the future
...
pub fn verify_transaction(tx: &Transaction) -> Result<(), ConsensusError> {
    ...
    if tx_size > max_size {
        return Err(ConsensusError::TransactionViolation(
            format!("Transaction too large: {} > {}", tx_size, max_size)
        ));
    }
...
pub fn check_consensus_rules_accelerated(block: &Block, prev_hash: &BlockHash) -> Result<(), ConsensusError> {
