    ...
    fn shr(self, rhs: u32) -> Self::Output {
        ...
        if bits_shift == 0 {
            ...
            for i in bytes_shift..32 {
                ...
                let high_bits = if src_idx > 0 { self.0[src_idx - 1] << (8 - bits_shift) } else { 0 };
...
impl std::fmt::Display for Uint256 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "0x{}..{}", hex::encode(&self.0[0..4]), hex::encode(&self.0[28..32]))
    }
}
...
impl ConsensusParams {
    ...
    pub fn signet() -> Self {
        ConsensusParams {
            ...
            taproot_active: true,
            taproot_activation_height: 43_, // Signet Taproot activation height
            custom_params: HashMap::new(),
        }
    }
...
impl ConsensusParamsProvider {
    ...
    pub fn with_custom_params(network: Network, custom_fn: impl FnOnce(&mut ConsensusParams)) -> Self {
...
mod tests {
    ...
    fn test_consensus_parameters() {
        ...
        assert_eq!(params.get_custom_param("max_ancestors"), Some(&"25".to_string()));
    }
    ...
    fn test_consensus_params_provider() {
        ...
        let custom_provider = ConsensusParamsProvider::with_custom_params(
            Network::Testnet,
            |p| p.max_block_weight = 8_000_000
        );
