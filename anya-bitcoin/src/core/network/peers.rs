...
impl PeerManager {
    ...
    pub fn with_config(max_peers: usize, max_outbound: usize, 
                      connect_timeout: Duration, is_testnet: bool,
                      taproot_enabled: bool) -> Self {
    ...
    pub async fn connect(&self, addr: SocketAddr, connection_type: ConnectionType) -> AnyaResult<String> {
        // Check if we've reached the peer limit
        {
            ...
            if connection_type == ConnectionType::Outbound {
                let outbound_count = peers.values()
                    .filter(|info| info.is_outbound())
                    .count();
                
                if outbound_count >= self.max_outbound {
                    return Err(PeerError::General("Max outbound connections reached".to_string()).into());
                }
            }
        }
    ...
    pub async fn register_handshake(&self, peer_id: &str, version: u32, user_agent: &str, services: u64) -> AnyaResult<bool> {
        let mut peers = self.peers.write().unwrap();
        ...
        if let Some(info) = peers.get_mut(peer_id) {
            ...
            info!("Handshake completed with peer {} (version: {}, user agent: '{}')", 
                 peer_id, version, user_agent);
            
            Ok(true)
        } else {
            warn!("Attempted to register handshake for unknown peer {}", peer_id);
            Ok(false)
        }
    }
