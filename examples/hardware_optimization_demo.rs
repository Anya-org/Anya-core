...
use anya_core::hardware_optimization::{
    self, Operation, OptimizationError, ExecutionError, WorkloadProfile,
    Priority, MemoryTarget, PowerTarget, HardwareOptimizationManager
};
use anya_core::hardware_optimization::integration::create_accelerated_optimizer;
use anya_core::hardware_optimization::benchmark::{BenchmarkSuite, BenchmarkSettings};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("  SHA-256 (1KB): {} iterations in {:.2?} ({:.2} hashes/sec)",
    ...
    let start = Instant::now();
    let schnorr_path = manager.optimize_operation(Operation::SchnorrVerification).await?;
    for _ in 0..ITERATIONS {
        let _ = schnorr_path.execute(&valid_sig).await?;
    }
    let elapsed = start.elapsed();
    println!("  Schnorr Verification: {} iterations in {:.2?} ({:.2} verifications/sec)",
             ITERATIONS, elapsed, ITERATIONS as f64 / elapsed.as_secs_f64());
    
    // Batch verification benchmark (most likely to benefit from GPU/NPU)
    let start = Instant::now();
    let batch_path = manager.optimize_operation(Operation::BatchVerification).await?;
    ...
    println!("  Batch Verification: {} signatures in {:.2?} ({:.2} sigs/sec)",
             BATCH_SIZE, elapsed, BATCH_SIZE as f64 / elapsed.as_secs_f64());
    println!("    Valid: {}, Invalid: {}", valid_count, invalid_count);
    
    // Run comprehensive benchmark suite
    println!("\n📈 Running comprehensive benchmark suite...");
    let benchmark_suite = BenchmarkSuite::new(Arc::new(manager.clone())).await
    ...
    benchmark_suite.save_report(&benchmark_report, &report_path).await?;
...
async fn print_performance_comparison(manager: &HardwareOptimizationManager) -> Result<(), OptimizationError> {
    println!("\n📊 Performance Comparison:");
    
    // Create generic fallback optimizer for baseline
    let generic_capabilities = manager.get_capabilities().await;
    let generic_optimizer = hardware_optimization::fallback::GenericOptimizer::new(&generic_capabilities).await?;
