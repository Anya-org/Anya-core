...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result =
        process_secure_data(&auth_manager, &web5_integration, &ml_features, &monitoring).await?;

    // Example 2: ML Analysis with Web5 Storage
    let analysis =
        analyze_and_store_data(&auth_manager, &web5_integration, &ml_features, &monitoring).await?;

    // Example 3: Monitored Blockchain Operations
    let blockchain_result =
        perform_blockchain_operations(&auth_manager, &web5_integration, &monitoring).await?;
...
async fn process_secure_data(
    ...
    // Process with ML
    let ml_result = ml_features.process_data(&test_data).await?;

    // Store in Web5 with monitoring
    monitoring
        .track_operation(OperationType::Storage, || {
...
async fn analyze_and_store_data(
    ...
    let analysis = monitoring
        .track_operation(OperationType::Analysis, || {
            ml_features.analyze_data(&test_data).await
        })
        .await?;

    // Store in Web5
    let record = web5_integration.store_analysis_result(&analysis).await?;
...
async fn perform_blockchain_operations(
    ...
    // Sign message
    let signature = auth_manager.sign_message(&test_message).await?;

    // Store in Web5
    let record = web5_integration.store_signature(&signature).await?;

    // Monitor results
    monitoring.record_blockchain_operation(OperationType::Signing, &record);
