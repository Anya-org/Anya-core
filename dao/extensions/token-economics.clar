...
;; Distribution percentages (must add up to 100%;);(define-constant DEX-ALLOCATION-PERCENTAGE u30;) ;; 30% allocated to DEX
...
(define-constant ERR_ALREADY_INITIALIZED u1002;);(define-constant ERR_NOT_INITIALIZED u1003;);
(define-constant ERR_DISTRIBUTION_NOT_STARTED u1004;);(define-constant ERR_EXCEEDS_AVAILABLE u1005;);
(define-constant ERR_INVALID_PHASE u1006;);(define-constant ERR_ZERO_AMOUNT u1007;);
(define-constant ERR_INSUFFICIENT_BALANCE u1008;)

;; SIP-010 Token Trait
(define-trait ft-trait
  (
    (transfer (uint principal principal (optional (buff 34;););); (response bool uint;););    (get-name (;) (response (string-ascii 32;) uint;););    (get-symbol (;) (response (string-ascii 32;) uint;););    (get-decimals (;) (response uint uint;););    (get-balance (principal;) (response uint uint;););    (get-total-supply (;) (response uint uint;););    (get-token-uri (;) (response (optional (string-utf8 256;);) uint;););    (mint (uint principal;) (response bool uint;););    (burn (uint principal;) (response bool uint;););)
;)

;; Data Variables - Token Distribution
(define-data-var distribution-start-block uint u0;);
(define-data-var tokens-distributed uint u0;);(define-data-var buyback-reserve uint u0;);
(define-data-var dex-liquidity-reserve uint u0;);(define-data-var initial-phase-released uint u0;);
...
(define-public (initialize-distribution (start-block uint;););
    (begin
        (asserts! (is-authorized tx-sender;) (err ERR_UNAUTHORIZED;););        (asserts! (is-eq (var-get distribution-start-block;) u0;) (err ERR_ALREADY_INITIALIZED;);)
        
        ;; Set distribution start
        (var-set distribution-start-block start-block;)
        
        ;; Initialize initial phase (45% in first 6 months;);        (map-set distribution-phases u1 {
...
(define-read-only (get-current-phase;);
    (let (
        (current-block block-height;);        (distribution-start (var-get distribution-start-block;););        (phase-1 (unwrap-panic (map-get? distribution-phases u1;);););        (phase-2 (unwrap-panic (map-get? distribution-phases u2;););)
...
(define-read-only (get-initial-phase-available;);
    (let (
        (current-block block-height;);        (phase-1 (unwrap-panic (map-get? distribution-phases u1;);););        (start-block (get start-block phase-1;););        (end-block (get end-block phase-1;););        (tokens-allocated (get tokens-allocated phase-1;););        (tokens-released (get tokens-released phase-1;);););        (if (> current-block end-block;)
            ;; Phase 1 is complete, return remaining allocation if any
            (if (< tokens-released tokens-allocated;);                (- tokens-allocated tokens-released;)
                u0;)
            ;; Phase 1 is still active, calculate based on block progression
            (let (
                (blocks-elapsed (- current-block start-block;););                (total-phase-blocks (- end-block start-block;););                (expected-release (/ (* tokens-allocated blocks-elapsed;) total-phase-blocks;);););                (if (< tokens-released expected-release;);                    (- expected-release tokens-released;)
                    u0;)
;)
;)
;)
...
(define-read-only (get-regular-phase-available;);
    (let (
        (current-block block-height;);        (phase-2 (unwrap-panic (map-get? distribution-phases u2;);););        (start-block (get start-block phase-2;););        (tokens-allocated (get tokens-allocated phase-2;););        (tokens-released (get tokens-released phase-2;);););        (if (< current-block start-block;)
            ;; Regular phase hasn't started yet
            u0
            ;; Linear release after initial phase
            (let (
                (blocks-elapsed (- current-block start-block;);)
                ;; Approximately 10% per year after initial phase (adjust as needed;);                (annual-blocks u1051200;) ;; ~52560 blocks * 20 years
                (expected-release (/ (* tokens-allocated blocks-elapsed;) annual-blocks;);););                (if (< tokens-released expected-release;);                    (- expected-release tokens-released;)
                    u0;)
;)
;)
;)
...
(define-read-only (get-available-to-mint;);
    (let (
        (current-block block-height;);        (distribution-start (var-get distribution-start-block;););        (total-distributed (var-get tokens-distributed;);););        (if (< current-block distribution-start;)
...
(define-private (update-initial-phase-distribution (amount uint;););
    (let (
        (phase-1 (unwrap-panic (map-get? distribution-phases u1;);););        (tokens-released (get tokens-released phase-1;);););        (map-set distribution-phases u1 (merge phase-1 { tokens-released: (+ tokens-released amount;) };);)
        
        ;; Check if we need to transition to regular phase
        (if (>= (+ tokens-released amount;) (get tokens-allocated phase-1;););            (var-set is-initial-distribution false;)
            true;)
;)
;)
...
(define-private (update-regular-phase-distribution (amount uint;););
    (let (
        (phase-2 (unwrap-panic (map-get? distribution-phases u2;);););        (tokens-released (get tokens-released phase-2;);););        (map-set distribution-phases u2 (merge phase-2 { tokens-released: (+ tokens-released amount;) };);)
        true;)
;)
...
(define-public (set-buyback-reserve (amount uint;););
    (begin
        (asserts! (is-authorized tx-sender;) (err ERR_UNAUTHORIZED;););        (var-set buyback-reserve amount;);        (ok true;)
;)
;)
...
(define-public (set-dex-liquidity-reserve (amount uint;););
    (begin
        (asserts! (is-authorized tx-sender;) (err ERR_UNAUTHORIZED;););        (var-set dex-liquidity-reserve amount;);        (ok true;)
;)
;)
...
(define-public (set-token-contract (new-token-contract principal;););
    (begin
        (asserts! (is-authorized tx-sender;) (err ERR_UNAUTHORIZED;););        (var-set token-contract new-token-contract;);        (ok true;)
;)
;)
