...
(define-constant ERR_INVALID_PROPOSAL (err u1001;););(define-constant ERR_PROPOSAL_EXISTS (err u1002;););
(define-constant ERR_VOTING_CLOSED (err u1003;););(define-constant ERR_INSUFFICIENT_BALANCE (err u1004;);)
...
(define-public (create-proposal (title (string-utf8 256;);); (description (string-utf8 4096;);) (duration uint;););    (let
        (
            (proposal-id (var-get proposal-count;););            (caller tx-sender;);            (token-balance (contract-call? .governance-token get-balance caller;););            (current-block block-height;)
;)
...
(define-public (vote (proposal-id uint;); (vote bool;););    (let
        (
            (caller tx-sender;);            (token-balance (contract-call? .governance-token get-balance caller;););            (proposal (unwrap! (map-get? proposals { proposal-id: proposal-id };) ERR_INVALID_PROPOSAL;););            (current-block block-height;)
;)
...
(define-public (execute-proposal (proposal-id uint;););
    (let
        (
            (proposal (unwrap! (map-get? proposals { proposal-id: proposal-id };) ERR_INVALID_PROPOSAL;););            (current-block block-height;)
;)
        ...
        (if (<= current-block (get end-block proposal;);)
            ...
            (begin
                ...
                (if (> (get yes-votes proposal;) (get no-votes proposal;););                    (begin
